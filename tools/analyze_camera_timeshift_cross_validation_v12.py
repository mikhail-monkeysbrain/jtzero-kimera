...
HOME = "/home/vio"
MANIFEST = os.path.join(HOME, "jtzero_camera_timeshift_cv_manifest.csv")
DETAIL = os.path.join(HOME, "jtzero_camera_timeshift_cv_detail.csv")
SUMMARY = os.path.join(HOME, "jtzero_camera_timeshift_cv_summary.csv")
TOP = os.path.join(HOME, "jtzero_camera_timeshift_cv_top.txt")
...
def main():
    ...
    for m in manifest:
        ...
        if key not in leg_cache:
            leg_cache[key] = load_leg_windows(m["legs"], load_kf_times(m["backend"]))
        ...
        rows.append({
            "dataset": m["dataset"],
            "shift_ms": float(m["shift_ms"]),
            "tag": m["tag"],
            **mm,
        })

    if not rows:
        raise SystemExit("no timeshift results")

    detail_fields = [
        "dataset","shift_ms","tag","score_mm","z_rms_mm","xy_err_rms_mm",
        "pair_closure_rms_mm","pair_xy_closure_rms_mm","xy_mean_mm","final_dp_mm",
    ]
    ...
    for r in rows:
        grouped[r["shift_ms"]].append(r)

    agg = []
    for shift, rr in grouped.items():
        agg.append({
            "shift_ms": shift,
            "datasets": len(rr),
    ...
    fields = [
        "rank","shift_ms","datasets","mean_score_mm","worst_score_mm",
        "mean_z_rms_mm","worst_z_rms_mm","mean_xy_rms_mm","worst_xy_rms_mm",
        "mean_pair_closure_mm","worst_pair_closure_mm","mean_final_dp_mm","worst_final_dp_mm",
    ]
    ...
    lines = []
    lines.append("================ CAMERA TIMESHIFT CROSS-VALIDATION ================")
    lines.append("shift_ms = additional offset added to recorded corrected camera timestamps")
    lines.append("recorded timestamps already contain +10.5 ms correction")
    lines.append("rank shift_ms  worst_score mean_score worst_Z worst_XY worst_pair worst_final")
    for i,r in enumerate(agg,1):
        lines.append(
            f'{i:>2} {r["shift_ms"]:>8.1f} {r["worst_score_mm"]:>11.2f} '
            f'{r["mean_score_mm"]:>10.2f} {r["worst_z_rms_mm"]:>7.2f} '
            f'{r["worst_xy_rms_mm"]:>8.2f} {r["worst_pair_closure_mm"]:>10.2f} '
            f'{r["worst_final_dp_mm"]:>11.2f}'
        )
    lines.append("")
    lines.append("Per-dataset detail:")
    lines.append("dataset shift_ms   score    Z_RMS   XY_RMS  pair_cl  XY_mean final_dP")
    for r in sorted(rows, key=lambda x:(x["shift_ms"],x["dataset"])):
        lines.append(
            f'{r["dataset"]:<7} {r["shift_ms"]:>8.1f} {r["score_mm"]:>8.2f} '

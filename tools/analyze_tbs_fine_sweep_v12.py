...
HOME = "/home/vio"
MANIFEST = os.path.join(HOME, "jtzero_tbs_fine_v12_manifest.csv")
BACKEND = os.path.join(HOME, "jtzero_500mm_v12_backend.csv")
LEGS = os.path.join(HOME, "jtzero_500mm_v12_legs.csv")
OUT = os.path.join(HOME, "jtzero_tbs_fine_v12_ranked.csv")
...
def nearest(states, t):
    # State count is only a few hundred; a linear scan is sufficient and robust.
...
def metrics(states, legs):
    ...
    # All terms are millimetres. Equal weighting prevents choosing a solution
    # that fixes Z by destroying metric XY or A/B reversibility.
...
def main():
    ...
    if not rows:
        raise SystemExit("no fine-sweep results found")

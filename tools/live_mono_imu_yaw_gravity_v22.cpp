...
constexpr double kYawTol = 2.0, kGyroStill = 0.08, kAccTol = 0.35;
...
static void hud(const cv::Mat& gray,const Telemetry& tel,bool ready,bool done,int phase,int stable,const Log22& log,double ref){
  ...
  cv::Mat s=c(cv::Rect(860,70,560,850));drawBar(s,"YAW",yy,-60,60,target,kYawTol,145);char b[256];snprintf(b,sizeof(b),"Этап %d/4   стабильность %d/%d",phase+1,stable,kStable);uiText(s,b,18,210,.42,white,1);snprintf(b,sizeof(b),"|gyro| %.3f   |acc| %.3f",gn,an);uiText(s,b,18,250,.42,white,1);

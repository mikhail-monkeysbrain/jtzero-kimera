// JT-ZERO v40: read-only dump параметров калибровки акселерометров ArduPilot.
// Никакие параметры FC не изменяются.

#define main jtzero_dump_unused_main_v40
#include "camera_imu_extrinsics_logger.cpp"
#undef main

namespace {

bool wanted40(const std::string& n) {
  // Main accel calibration parameters for IMU1/2/3.
  if (n.rfind("INS_ACC",0)==0) return true;
  // Per-IMU temperature calibration groups.
  if (n.rfind("INS_TCAL",0)==0) return true;
  // Sensor IDs/use are useful for instance mapping.
...
void requestList40(int fd,uint8_t sys,uint8_t comp){
...
std::string pname40(const mavlink_param_value_t& p){
  char b[17]{}; std::memcpy(b,p.param_id,16);
  ...
  try{
    ...
    while(monotonicNs()<deadline){
      ...
      for(ssize_t i=0;i<n;++i){
        ...
        std::string name=pname40(pv);
        if(wanted40(name)) vals[name]=pv.param_value;
      }
    }

    std::cout<<"\n================ КАЛИБРОВКА АКСЕЛЕРОМЕТРОВ FC v40 ================\n";
    std::cout<<"Всего параметров FC: "<<expected<<"\n";
    std::cout<<"Выбрано параметров: "<<vals.size()<<"\n\n";
    for(const auto& kv:vals){
      std::cout<<std::left<<std::setw(22)<<kv.first<<" = "<<std::setprecision(10)<<kv.second<<"\n";
    }
    std::cout<<"\nREAD-ONLY: параметры не изменялись.\n";
    close(fd); return 0;

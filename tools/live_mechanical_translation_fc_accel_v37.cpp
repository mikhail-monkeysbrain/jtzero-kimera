// JT-ZERO v37: external mechanical reference test for 500 mm translation.
// No Kimera. Logs FC ATTITUDE + HIGHRES_IMU while operator records side video.
// Required geometry: stand/drone yaw approximately -90 deg.
// Sequence: settle A -> record A->B -> settle B -> record B->A -> settle A.
...
constexpr const char* kWindow="JT-ZERO: MECHANICAL TRANSLATION v37";
...
static const char* phaseName(int p){
  switch(p){
    case 0:return "SETTLE_A_START";
    case 1:return "MOVE_A_TO_B";
    case 2:return "SETTLE_B";
    case 3:return "MOVE_B_TO_A";
    case 4:return "SETTLE_A_END";
    default:return "DONE";
  }
}
...
static void hud(const Att&a,const Eigen::Vector3d&acc,const Eigen::Vector3d&gyro,int phase,bool recording){
  ...
  uiText(c,"JT-ZERO: MECHANICAL TRANSLATION v37",30,55,.72,w,2);
  uiText(c,"DRONE/STAND YAW: approximately -90 deg",45,115,.62,ye,2);
  uiText(c,"SIDE VIDEO: fixed phone; show FC marker AND rigid upper platform",45,165,.50,w,1);
  char t[256];
  snprintf(t,sizeof(t),"PHASE %d/5: %s",std::min(phase+1,5),phaseName(phase));
  uiText(c,t,45,235,.62,recording?gr:ye,2);
  if(phase==0) uiText(c,"Hold at A. SPACE starts 3 s settle capture.",45,295,.50,w,1);
  if(phase==1) uiText(c,"Move A -> B exactly 500 mm. SPACE when fully settled at B.",45,295,.50,w,1);
  if(phase==2) uiText(c,"Hold at B. SPACE begins B -> A.",45,295,.50,w,1);
  if(phase==3) uiText(c,"Move B -> A exactly 500 mm. SPACE when fully settled at A.",45,295,.50,w,1);
  if(phase==4) uiText(c,"Hold at A. SPACE finishes after at least 3 s.",45,295,.50,w,1);
  snprintf(t,sizeof(t),"FC R/P/Y = %+.3f  %+.3f  %+.3f deg",a.roll,a.pitch,a.yaw);
  ...
  uiText(c,"Do not change yaw/height/rig configuration during the whole test.",45,600,.48,red,1);
  uiText(c,"Keep recording the phone video continuously until DONE.",45,655,.48,red,1);
  cv::imshow(kWindow,c);
}
}
...
int main(int argc,char**argv){
  ...
  try{
    fd=openSerial(); std::cout<<"[MAV] waiting for HEARTBEAT...\n";
    ...
    while(true){
      ...
      if(k==' ' && at.valid){
        if(!recording){
          ...
          std::cout<<"[PHASE START] "<<jtzero_v37::phaseName(phase)<<" wall_ns="<<phase_start<<"\n";
        }else{
          const double sec=(monotonicNs()-phase_start)/1e9;
          if((phase==0||phase==2||phase==4) && sec<3.0){
            std::cout<<"[WAIT] stationary phase needs at least 3.0 s; current="<<sec<<"\n";
            continue;
          }
          std::cout<<"[PHASE END] "<<jtzero_v37::phaseName(phase)<<" duration="<<sec<<" s\n";
          recording=false; phase++;
          if(phase>=5){ std::cout<<"[TEST] DONE\n"; break; }

// JT-ZERO v12 T_BS rotation sweep replay.
// Replays one recorded v12 Camera+IMU dataset with the same static-only gravity
// feedback gating as the live 500 mm test, while applying an orientation-only
// correction to camera->body extrinsics. Translation is intentionally unchanged.
...
constexpr const char* kDefaultCombined = "/home/vio/jtzero_500mm_v12.csv";
constexpr const char* kDefaultCamIndex = "/home/vio/jtzero_500mm_v12_camera.csv";
constexpr const char* kDefaultMjpeg = "/home/vio/jtzero_500mm_v12.mjpg";
constexpr const char* kDefaultBackend = "/home/vio/jtzero_500mm_v12_backend.csv";
constexpr const char* kDefaultLegs = "/home/vio/jtzero_500mm_v12_legs.csv";
...
std::map<int64_t,int64_t> loadBackendKfTimes(const std::string& path) {
  std::ifstream f(path); if(!f) throw std::runtime_error("Cannot open backend CSV: "+path);
  std::map<int64_t,int64_t> out; std::string line; std::getline(f,line);
  while(std::getline(f,line)){
    auto v=splitCsv10(line); if(v.size()<4) continue;
    out[i64v10(v[2])] = i64v10(v[3]);
  }
  if(out.empty()) throw std::runtime_error("No backend timestamps");
  return out;
}

void loadLegWindows(const std::string& legs_path,
                    const std::map<int64_t,int64_t>& kf_time,
                    std::vector<GateWindow>* gates,
                    std::vector<LegWindow>* legs) {
  std::ifstream f(legs_path); if(!f) throw std::runtime_error("Cannot open legs CSV: "+legs_path);
  std::string line; std::getline(f,line);
  while(std::getline(f,line)){
    auto v=splitCsv10(line); if(v.size()<6) continue;
    const int leg=std::stoi(v[0]);
    const std::string dir=v[1];
    const int64_t start_settled=i64v10(v[3]);
    const int64_t end_press=i64v10(v[4]);
    const int64_t end_settled=i64v10(v[5]);
    auto a=kf_time.find(start_settled), b=kf_time.find(end_press), e=kf_time.find(end_settled);
    if(a==kf_time.end()||b==kf_time.end()||e==kf_time.end())
      throw std::runtime_error("Missing KF timestamp for leg "+std::to_string(leg));
    gates->push_back({leg,a->second,b->second});
    legs->push_back({leg,dir,a->second,e->second});
  }
  if(legs->empty()) throw std::runtime_error("No leg windows");
}

bool allowGravityAt(int64_t mapped_ns,const std::vector<GateWindow>& gates){
  for(const auto& g:gates) if(mapped_ns>=g.start_ns && mapped_ns<=g.end_ns) return false;
  return true;
}

const VioState* nearestState(const std::vector<VioState>& s,int64_t t){
  if(s.empty()) return nullptr;
  const VioState* best=&s.front();
  int64_t bd=std::llabs(best->timestamp_ns-t);
  for(const auto& q:s){
    const int64_t d=std::llabs(q.timestamp_ns-t);
    if(d<bd){bd=d;best=&q;}
  }
  return best;
}

void printLegMetrics(const std::vector<VioState>& s,const std::vector<LegWindow>& legs){
  double z2=0.0, xyerr2=0.0, sumxy=0.0; int n=0;
  std::cout<<"--- LEG METRICS ---\n";
  for(const auto& w:legs){
    const auto*a=nearestState(s,w.start_ns); const auto*b=nearestState(s,w.end_ns);
    if(!a||!b) continue;
    const double dx=b->px-a->px,dy=b->py-a->py,dz=b->pz-a->pz;
    const double xy=std::sqrt(dx*dx+dy*dy);
    const double err=xy-0.500;
    z2+=dz*dz; xyerr2+=err*err; sumxy+=xy; ++n;
    std::cout<<"LEG "<<w.leg<<" "<<w.direction
             <<" XY="<<std::fixed<<std::setprecision(2)<<xy*1000.0
             <<"mm dZ="<<dz*1000.0
             <<"mm err="<<err*1000.0<<"mm\n";
  }
  if(n){
    std::cout<<"LEG_Z_RMS_MM="<<std::sqrt(z2/n)*1000.0
             <<" LEG_XY_ERR_RMS_MM="<<std::sqrt(xyerr2/n)*1000.0
             <<" LEG_XY_MEAN_MM="<<sumxy/n*1000.0<<"\n";
  }
}
...
int main(int argc,char**argv){
  ...
  const std::string tag=argc>2?argv[2]:"TBS";
  const double corr_roll_deg=argc>3?std::stod(argv[3]):0.0;
  const double corr_pitch_deg=argc>4?std::stod(argv[4]):0.0;
  const std::string combined=argc>5?argv[5]:kDefaultCombined;
  const std::string camindex=argc>6?argv[6]:kDefaultCamIndex;
  const std::string mjpeg=argc>7?argv[7]:kDefaultMjpeg;
  const std::string backend=argc>8?argv[8]:kDefaultBackend;
  const std::string legs_path=argc>9?argv[9]:kDefaultLegs;
  ...
  try{
    ...
    const auto kf_time=loadBackendKfTimes(backend);std::vector<GateWindow> gates;std::vector<LegWindow> leg_windows;
    ...
    const std::string out="/home/vio/jtzero_extrinsics_replay_v10_"+tag+".csv";saveStates10(out,states);summary10(tag,params,tbc,states,imu.size(),raw,rej,cam.size(),decoded);printLegMetrics(states,leg_windows);std::cout<<"states CSV: "<<out<<"\n";return states.empty()?1:0;

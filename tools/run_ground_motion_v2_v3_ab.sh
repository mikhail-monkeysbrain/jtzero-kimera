...
BIN="/tmp/jtzero_ground_motion_v2_v3_ab"
...
OUT="${JTZERO_GM_OUT:-$HOME/jtzero_runs/${STAMP}_GROUND_MOTION_V2_V3_AB.csv}"
mkdir -p "$(dirname "$OUT")"
if [[ ! -x "$BIN" || "$ROOT/tools/ground_motion_live_v2_v3_ab.cpp" -nt "$BIN" ]]; then
  bash "$ROOT/tools/build_ground_motion_v2_v3_ab.sh" "$BIN"
fi
echo "======================================================================"
echo "JT-ZERO GROUND MOTION V2/V3 A/B"

...
int64_t frameTvToNsReplayRecord(const timeval& tv) {
    return (int64_t)tv.tv_sec * 1000000000LL + (int64_t)tv.tv_usec * 1000LL;
}

double wrapReplayRecord(double a) {
    while (a > kPi) a -= 2 * kPi;
    while (a < -kPi) a += 2 * kPi;
    return a;
}
...
struct CaptureFcHistory {
    ...
    void start(const std::string& dev) {
        ...
        th = std::thread([this] {
            ...
            if (!sys) {
                std::cerr << "FC: ArduPilot HEARTBEAT timeout\n";
                g_running = false;
                return;
            }
            target_sys = sys; target_comp = comp;
            std::cerr << "FC: ArduPilot heartbeat sys=" << (int)sys
                      << " comp=" << (int)comp << "\n";
            ...
            while (g_running) {
                ...
                for (;;) {
                    ...
                    for (ssize_t i = 0; i < n; ++i) {
                        if (!mavlink_parse_char(MAVLINK_COMM_0, buf[i], &m, &st)) continue;
                        if (m.msgid != MAVLINK_MSG_ID_ATTITUDE || m.sysid != sys) continue;
                        mavlink_attitude_t a{};
                        mavlink_msg_attitude_decode(&m, &a);
                        Attitude x{a.roll, a.pitch, a.yaw, monoNs(), true};
                        std::lock_guard<std::mutex> l(mu);
                        hist.push_back(x);
                        const int64_t keep = x.recv_ns - 3000000000LL;
    ...
    bool at(int64_t target, Attitude* out, double* nearest_ms) {
        ...
        size_t hi = 1;
        while (hi < hist.size() && hist[hi].recv_ns < target) ++hi;
        if (hi >= hist.size()) return false;
        const auto& a = hist[hi - 1];
        const auto& b = hist[hi];
        const double u = double(target - a.recv_ns) / double(b.recv_ns - a.recv_ns);
        out->roll = a.roll + u * (b.roll - a.roll);
        out->pitch = a.pitch + u * (b.pitch - a.pitch);
        out->yaw = wrapReplayRecord(a.yaw + u * wrapReplayRecord(b.yaw - a.yaw));
        out->recv_ns = target;
        out->valid = true;
        if (nearest_ms) {
            *nearest_ms = std::min(std::llabs(target - a.recv_ns),
                                   std::llabs(b.recv_ns - target)) * 1e-6;
        }
        return true;
    }
...
int main(int argc, char** argv) {
    if (argc < 7) {
        std::cerr << "Использование: " << argv[0]
                  << " <camera> <luna> <fc> <out_dir> <camera_yaml> <camera_offset_mm>\n";
        return 2;
    }

    const std::string camdev = argv[1];
    const std::string lunadev = argv[2];
    const std::string fcdev = argv[3];
    const std::filesystem::path outdir = argv[4];
    const std::string yaml = argv[5];
    ...
    try {
        ...
        CaptureFcHistory fc; fc.start(fcdev);

        std::ofstream payload(outdir / "frames.mjpgbin", std::ios::binary | std::ios::trunc);
        ...
        meta << "frame,mono_ns,ts_ns,offset_bytes,size_bytes,decode_ok,luna_valid,luna_m,luna_age_ms,"
                "att_valid,att_age_ms,roll,pitch,yaw,height_m,sensors_valid\n";
        ...
        while (g_running) {
            ...
            while (g_running) {
                ...
                const int64_t ts = frameTvToNsReplayRecord(b.timestamp);
                std::vector<uint8_t> raw((uint8_t*)cam.bufs[b.index].p,
                                         (uint8_t*)cam.bufs[b.index].p + b.bytesused);
                if (xioctl(cam.fd, VIDIOC_QBUF, &b) < 0) fail("VIDIOC_QBUF");
                ++frame;

                const uint64_t off = (uint64_t)payload.tellp();
                payload.write(reinterpret_cast<const char*>(raw.data()), (std::streamsize)raw.size());
                if (!payload) throw std::runtime_error("ошибка записи frames.mjpgbin");

                cv::Mat one(1, (int)raw.size(), CV_8UC1, raw.data());
                cv::Mat gray = cv::imdecode(one, cv::IMREAD_GRAYSCALE);
                const bool decode_ok = !gray.empty();

                double lm = 0.0; int strength = 0; int64_t lns = 0;
                Attitude att{}; double att_age = 1e9;
                const bool hl = luna.latest(&lm, &strength, &lns);
                const bool ha = fc.at(ts, &att, &att_age);
                const double lage = hl ? (now - lns) * 1e-6 : 1e9;
                double h = 0.0, down = 0.0;
                if (hl && ha) {
                    const auto R = attitudeFluToNwu(att);
                    down = -(R * cv::Vec3d(0, 0, -1))[2];
                    h = lm * down - offset + (R * lever_b)[2];
                }
                const bool sensors = decode_ok && hl && ha && down > 0.20 && h > 0.05 &&
                                     lage < 200.0 && att_age < 30.0;

                meta << frame << ',' << now << ',' << ts << ',' << off << ',' << raw.size() << ','
                     << (decode_ok ? 1 : 0) << ',' << (hl ? 1 : 0) << ',' << lm << ',' << lage << ','
                     << (ha ? 1 : 0) << ',' << att_age << ','
                     << att.roll << ',' << att.pitch << ',' << att.yaw << ',' << h << ','
                     << (sensors ? 1 : 0) << '\n';

                if ((frame % 100) == 0) {
                    meta.flush(); payload.flush();
                    std::cerr << "REC frame=" << frame << " bytes=" << off + raw.size()
                              << " sensors=" << (sensors ? 1 : 0) << "\r" << std::flush;
                }
            }
        }

...
constexpr const char* kCsvPath = "/home/vio/jtzero_live_500mm_hud.csv";
constexpr const char* kWindowName = "JT-ZERO 500 mm LIVE HUD";
...
struct ClockMapping {
  ...
  int64_t map(const int64_t fc_ns) const {
...
double nsToMs(const int64_t ns) { return static_cast<double>(ns) / 1e6; }
...
class HudPipeline final : public VIO::MonoImuPipeline {
 public:
  explicit HudPipeline(const VIO::VioParams& params)
    ...
    registerBackendOutputCallback(
        [this](const std::shared_ptr<VIO::BackendOutput>& out) {
          ...
          if (!states_.empty()) {
            ...
            const double dx = s.px - prev.px;
            const double dy = s.py - prev.py;
            const double dz = s.pz - prev.pz;
            const double dp = std::sqrt(dx*dx + dy*dy + dz*dz);
            const double dt_s = static_cast<double>(s.timestamp_ns - prev.timestamp_ns) / 1e9;
            if (dp >= kJumpThresholdM && dt_s > 0.0 && dt_s < 1.0) {
              last_jump_.valid = true;
              last_jump_.keyframe = s.keyframe;
              last_jump_.dp_mm = dp * 1000.0;
              last_jump_.dt_ms = dt_s * 1000.0;
              last_jump_.speed_mm_s = dp / dt_s * 1000.0;
              last_jump_.droll_deg = wrapDeg(s.roll_deg - prev.roll_deg);
              last_jump_.dpitch_deg = wrapDeg(s.pitch_deg - prev.pitch_deg);
              last_jump_.dyaw_deg = wrapDeg(s.yaw_deg - prev.yaw_deg);
              last_jump_.wall_ns = monotonicNs();
              std::cout << std::fixed << std::setprecision(3)
                        << "[VIO-JUMP] kf=" << s.keyframe
                        << " dP=" << last_jump_.dp_mm << "mm"
                        << " dt=" << last_jump_.dt_ms << "ms"
                        << " equiv_speed=" << last_jump_.speed_mm_s << "mm/s"
                        << " dRPY=[" << last_jump_.droll_deg << ','
                        << last_jump_.dpitch_deg << ',' << last_jump_.dyaw_deg
                        << "]deg\n";
            }
          }
          states_.push_back(s);
  ...
  bool latest(VioState* out) const {
    std::lock_guard<std::mutex> lock(mutex_);
...
void writeCsv(const std::vector<VioState>& states,
  ...
  if (!f) throw std::runtime_error("Cannot create HUD CSV");
...
void putText(cv::Mat& img, const std::string& text, cv::Point p,
             double scale, cv::Scalar color, int thickness = 2) {
  cv::putText(img, text, p, cv::FONT_HERSHEY_SIMPLEX, scale, color,
              thickness, cv::LINE_AA);
}

cv::Scalar levelColor(double abs_ratio) {
  if (abs_ratio < 0.35) return cv::Scalar(80, 220, 80);
  if (abs_ratio < 0.70) return cv::Scalar(0, 220, 255);
  return cv::Scalar(40, 40, 245);
}

void drawGauge(cv::Mat& panel, const std::string& label,
               double value, double limit, const std::string& unit,
               int y) {
  const int x0 = 30;
  const int x1 = panel.cols - 30;
  const int center = (x0 + x1) / 2;
  const double q = std::max(-1.0, std::min(1.0, value / limit));
  const int xpos = static_cast<int>(center + q * (x1 - x0) * 0.5);
  const cv::Scalar c = levelColor(std::abs(q));

  char buf[128];
  std::snprintf(buf, sizeof(buf), "%s %+7.2f %s", label.c_str(), value, unit.c_str());
  putText(panel, buf, cv::Point(24, y - 20), 0.74, cv::Scalar(245,245,245), 2);
  cv::line(panel, cv::Point(x0, y + 5), cv::Point(x1, y + 5), cv::Scalar(130,130,130), 3);
  cv::line(panel, cv::Point(center, y - 9), cv::Point(center, y + 19), cv::Scalar(240,240,240), 2);
  cv::circle(panel, cv::Point(xpos, y + 5), 10, c, -1, cv::LINE_AA);
}

std::string currentPhase(int64_t now, int64_t move_ns, int64_t end_ns, int64_t stop_ns) {
  if (now < move_ns) return "START";
  if (now < end_ns) return "MOVE";
  if (now < stop_ns) return "END";
  return "DONE";
}
...
void renderHud(const cv::Mat& gray, const HudPipeline& pipeline,
               HudReference* ref, const std::string& phase,
               int64_t now_ns, double seconds_left) {
  cv::Mat bgr;
  cv::cvtColor(gray, bgr, cv::COLOR_GRAY2BGR);

  cv::Mat video;
  cv::resize(bgr, video, cv::Size(960, 720), 0, 0, cv::INTER_LINEAR);

  const int cx = video.cols / 2;
  const int cy = video.rows / 2;
  cv::line(video, cv::Point(cx - 45, cy), cv::Point(cx + 45, cy), cv::Scalar(0,255,255), 2);
  cv::line(video, cv::Point(cx, cy - 45), cv::Point(cx, cy + 45), cv::Scalar(0,255,255), 2);
  cv::circle(video, cv::Point(cx,cy), 12, cv::Scalar(0,255,255), 2, cv::LINE_AA);

  cv::Mat panel(900, 320, CV_8UC3, cv::Scalar(24,24,24));
  cv::Mat canvas(900, 1280, CV_8UC3, cv::Scalar(10,10,10));
  video.copyTo(canvas(cv::Rect(0, 90, 960, 720)));

  putText(canvas, "JT-ZERO LIVE 500 mm", cv::Point(28, 52), 1.15, cv::Scalar(245,245,245), 3);
  putText(canvas, "Camera view + alignment", cv::Point(28, 82), 0.65, cv::Scalar(190,190,190), 1);

  const cv::Scalar phase_color = phase == "MOVE" ? cv::Scalar(0,230,255) :
                                 phase == "END" ? cv::Scalar(90,220,90) :
                                                  cv::Scalar(245,245,245);
  putText(panel, "PHASE " + phase, cv::Point(22, 45), 0.9, phase_color, 3);
  char time_buf[64];
  std::snprintf(time_buf, sizeof(time_buf), "%.1f s left", std::max(0.0, seconds_left));
  putText(panel, time_buf, cv::Point(22, 78), 0.62, cv::Scalar(190,190,190), 1);

  VioState s;
  if (!pipeline.latest(&s)) {
    putText(panel, "Waiting for", cv::Point(22, 160), 0.85, cv::Scalar(230,230,230), 2);
    putText(panel, "Kimera state...", cv::Point(22, 198), 0.85, cv::Scalar(230,230,230), 2);
  } else {
    if (!ref->have_baseline) {
      ref->baseline = s;
      ref->have_baseline = true;
    }

    const double dx = s.px - ref->baseline.px;
    const double dy = s.py - ref->baseline.py;
    const double dz = s.pz - ref->baseline.pz;
    const double droll = wrapDeg(s.roll_deg - ref->baseline.roll_deg);
    const double dpitch = wrapDeg(s.pitch_deg - ref->baseline.pitch_deg);
    const double dyaw = wrapDeg(s.yaw_deg - ref->baseline.yaw_deg);
    const double h = std::sqrt(dx*dx + dy*dy);

    if (!ref->have_direction && phase == "MOVE" && h >= kDirectionLearnDistanceM) {
      ref->dir_x = dx / h;
      ref->dir_y = dy / h;
      ref->have_direction = true;
      std::cout << std::fixed << std::setprecision(5)
                << "[HUD] learned motion direction = ["
                << ref->dir_x << ',' << ref->dir_y << "]\n";
    }

    const double travel = ref->have_direction ? dx*ref->dir_x + dy*ref->dir_y : h;
    const double cross = ref->have_direction ? ref->dir_x*dy - ref->dir_y*dx : 0.0;
    const double speed = std::sqrt(s.vx*s.vx + s.vy*s.vy + s.vz*s.vz);

    drawGauge(panel, "ROLL ", droll, 5.0, "deg", 135);
    drawGauge(panel, "PITCH", dpitch, 5.0, "deg", 225);
    drawGauge(panel, "YAW  ", dyaw, 8.0, "deg", 315);
    drawGauge(panel, "Z    ", dz * 1000.0, 50.0, "mm", 405);
    drawGauge(panel, "CROSS", cross * 1000.0, 50.0, "mm", 495);

    char buf[128];
    std::snprintf(buf, sizeof(buf), "TRAVEL %.0f / 500 mm", travel*1000.0);
    putText(panel, buf, cv::Point(22, 590), 0.76, cv::Scalar(245,245,245), 2);

    const double progress = std::max(0.0, std::min(1.0, travel / kExpectedDistanceM));
    cv::rectangle(panel, cv::Rect(24, 612, 270, 24), cv::Scalar(80,80,80), 2);
    cv::rectangle(panel, cv::Rect(27, 615, static_cast<int>(264*progress), 18),
                  cv::Scalar(90,210,90), -1);

    std::snprintf(buf, sizeof(buf), "|V| %.1f mm/s", speed*1000.0);
    putText(panel, buf, cv::Point(22, 680), 0.72, cv::Scalar(225,225,225), 2);
    std::snprintf(buf, sizeof(buf), "KF %lld", static_cast<long long>(s.keyframe));
    putText(panel, buf, cv::Point(22, 715), 0.66, cv::Scalar(190,190,190), 1);

    const JumpEvent j = pipeline.lastJump();
    if (j.valid && now_ns - j.wall_ns < 3000000000LL) {
      cv::rectangle(panel, cv::Rect(12, 748, 296, 94), cv::Scalar(30,30,210), -1);
      putText(panel, "VIO JUMP", cv::Point(28, 785), 0.85, cv::Scalar(255,255,255), 3);
      std::snprintf(buf, sizeof(buf), "KF %lld  dP %.0f mm",
                    static_cast<long long>(j.keyframe), j.dp_mm);
      putText(panel, buf, cv::Point(28, 818), 0.58, cv::Scalar(255,255,255), 2);
    } else {
      putText(panel, "Keep gauges near center", cv::Point(22, 780), 0.58,
              cv::Scalar(170,170,170), 1);
      putText(panel, "ESC / Q = abort", cv::Point(22, 822), 0.62,
              cv::Scalar(210,210,210), 1);
    }
  }

  panel.copyTo(canvas(cv::Rect(960, 0, 320, 900)));
...
void printMeasurement(const MeanState& a, const MeanState& b) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "MEASUREMENT RESULT:     REPORT ONLY\n";
}
...
int main(int argc, char** argv) {
  ...
  bool pipeline_thread_started = false;
  ...
  try {
    ...
    std::cout << "\n============================================================\n"
              << "JT-ZERO GUIDED 500 MM TEST + INTEGRATED CAMERA HUD\n"
              << "============================================================\n"
              << "START: keep rig motionless for 10 s.\n"
              << "MOVE:  move exactly 500 mm after the explicit MOVE command.\n"
              << "END:   hold motionless for final 10 s.\n"
              << "Use the large camera image to guide the rig.\n"
              << "Keep ROLL/PITCH/YAW, Z and CROSS gauges near center.\n"
              << "Mechanical 500 mm marks remain the ground truth.\n"
              << "ESC/Q aborts the run. CSV: " << kCsvPath << "\n"
              << "============================================================\n";
    ...
    bool aborted = false;
    HudReference hud_ref;
    cv::Mat last_gray(kHeight, kWidth, CV_8UC1, cv::Scalar(0));

    while (monotonicNs() < stop_ns) {
      const int64_t now = monotonicNs();
      const std::string phase = currentPhase(now, move_ns, end_ns, stop_ns);
      const int64_t phase_end = now < move_ns ? move_ns : (now < end_ns ? end_ns : stop_ns);
      const double seconds_left = static_cast<double>(phase_end - now) / 1e9;
      ...
      int countdown = static_cast<int>(std::ceil(seconds_left));
      if (countdown != last_countdown && countdown <= 5 && countdown > 0) {
        last_countdown = countdown;
        std::cout << '[' << phase << "] " << countdown << " s remaining\n";
      }
      ...
      renderHud(last_gray, *pipeline, &hud_ref, phase, now, seconds_left);
      const int key = cv::waitKey(1) & 0xff;
      if (key == 27 || key == 'q' || key == 'Q') {
        aborted = true;
        std::cout << "[HUD] user abort requested\n";
        break;
      }
    }

    std::cout << "[500MM-HUD] stopping and draining...\n";
    ...
    const MeanState start_mean = meanWindow(states, start_avg_begin, move_ns);
    const MeanState end_mean = meanWindow(states, end_avg_begin, stop_ns);
    ...
    if (camera_fd != -1) { close(camera_fd); camera_fd = -1; }
    if (serial_fd != -1) { close(serial_fd); serial_fd = -1; }

    std::cout << "\n============================================================\n"
              << "JT-ZERO LIVE MONO+IMU 500 MM HUD RESULT\n"
              << "============================================================\n"
    ...
    const JumpEvent last_jump = pipeline->lastJump();
    std::cout << "VIO jump detected:      " << (last_jump.valid ? "yes" : "no") << "\n";
    if (last_jump.valid) {
      std::cout << "last jump KF:           " << last_jump.keyframe << "\n"
                << "last jump dP:           " << last_jump.dp_mm << " mm\n";
    }
    std::cout << "CSV:                    " << kCsvPath << "\n";

    const bool pipeline_pass = !aborted && mapping.valid &&
                               selected_frames >= total_sec * 20.0 &&
                               imu_fed >= total_sec * 150.0 &&
                               states.size() >= 80 &&
    ...
    if (pipeline_thread_started && pipeline_thread.joinable()) pipeline_thread.join();

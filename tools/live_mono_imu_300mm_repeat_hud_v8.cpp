...
constexpr const char* kV8CsvPath = "/home/vio/jtzero_live_300mm_repeat_v8.csv";
constexpr const char* kV8WindowName = "JT-ZERO 300 мм x6 v8 — stabilized endpoints";
...
bool v8MeasurementPass(const std::vector<V8Leg>& rows, double* sd_out=nullptr) {
  ...
  for (const auto& row : rows) {
    ...
    if (std::abs(r.error) > kV8MaxLegErrorM || std::abs(r.dz) > kV8MaxAbsDzM) return false;
    sum += r.horizontal;
...
void printSummaryV8(const std::vector<V8Leg>& rows,size_t raw,size_t rejected,
  ...
  if(!rows.empty()){
    ...
    std::cout << "MEAN HORIZONTAL: " << mean*1000.0 << " mm\n"
              ...
              << "MEAN SCALE measured/true: " << std::setprecision(6) << mean/kTruthM << "\n";
  }
...
void drawHudV8(const cv::Mat& gray,const HudPipeline& pipeline,const FcAttitude& fc,
  ...
  ru(canvas,"JT-ZERO — 300 мм × 6 — v8",{28,52},27,{245,245,245},cv::QT_FONT_BOLD);
  ru(canvas,"Fusion v7 + стабилизация START/END",{28,84},17,{190,190,190});
  char b[256];
  ...
  if(phase==CapturePhase::SETTLE_START||phase==CapturePhase::SETTLE_END){
    ...
    ru(panel,phase==CapturePhase::WAIT_START?"Установите камеру точно на метку":"Переместите камеру ровно на 300 мм",{18,355},14,{220,220,220});
  }
...
int main(int argc,char**argv){
  ...
  try{
    ...
    cv::Mat gray(kHeight,kWidth,CV_8UC1,cv::Scalar(0));jtzero::ImuCorrection imu_correction;
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(500));pipeline->shutdown();if(pipeline_thread.joinable())pipeline_thread.join();pipeline_started=false;cv::destroyAllWindows();const auto states=pipeline->states();saveCsvV8(rows);

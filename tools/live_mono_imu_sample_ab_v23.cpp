...
static void process23(Integrator23& in,int phase,const ImuSample10& s,
  ...
  Backend23 b;
  pipe.latest(&b);
  Eigen::Vector3d pvd=Eigen::Vector3d::Zero();
  if(b.valid)pvd=b.Vpred-in.pim_v0;
  const Eigen::Vector3d rf=pvd-in.vfc;
  const Eigen::Vector3d rg=pvd-in.vgyro;
  ...
  r.vfx=in.vfc.x();r.vfy=in.vfc.y();r.vfz=in.vfc.z();
  r.vgx=in.vgyro.x();r.vgy=in.vgyro.y();r.vgz=in.vgyro.z();
  r.keyframe=b.valid?b.keyframe:0;
  r.pvdx=pvd.x();r.pvdy=pvd.y();r.pvdz=pvd.z();
  r.resfx=rf.x();r.resfy=rf.y();r.resfz=rf.z();
  r.resgx=rg.x();r.resgy=rg.y();r.resgz=rg.z();
  in.rows.push_back(r);
}

static void save23(const Integrator23& in){
  std::ofstream f(kCsv23,std::ios::trunc);
...
static void hud23(const cv::Mat& gray,const Telemetry& tel,bool ready,bool done,
  ...
  if(gray.empty())v=cv::Mat(kHeight,kWidth,CV_8UC3,cv::Scalar(0));
  else cv::cvtColor(gray,v,cv::COLOR_GRAY2BGR);
  cv::resize(v,v,{820,615},0,0,cv::INTER_NEAREST);
  v.copyTo(c(cv::Rect(20,80,820,615)));
  uiText(c,"JT-ZERO: ПОКАДРОВЫЙ IMU A/B v23",20,48,.72,white,2);

  double yy=0,gn=0,an=0;
  {
    std::lock_guard<std::mutex>q(tel.mutex);
    yy=tel.fc_accum_yaw-ref;
  ...
  if(!ready){
    ...
    uiText(c,"SPACE — ЗАФИКСИРОВАТЬ НОЛЬ И BIAS",135,355,.70,yellow,2);
  }else if(done){
    uiText(c,"ТЕСТ ЗАВЕРШЁН",250,300,1.05,green,3);
  }else{
    char t[128];
    snprintf(t,sizeof(t),"ПОВЕРНИТЕ YAW К %+.0f° И ОСТАНОВИТЕСЬ",target);
    uiText(c,t,120,300,.70,yellow,3);
  }

  cv::Mat s=c(cv::Rect(860,70,560,850));
  drawBar(s,"YAW",yy,-60,60,target,kYawTol23,145);
  char b[256];
  ...
  if(!in.rows.empty()){
    ...
    const double vf=std::hypot(r.vfx,r.vfy);
    const double vg=std::hypot(r.vgx,r.vgy);
    const double vp=std::hypot(r.pvdx,r.pvdy);
    const double rf=std::hypot(r.resfx,r.resfy);
    const double rg=std::hypot(r.resgx,r.resgy);
...
static void printSummary23(const Integrator23& in){
  for(int p=0;p<4;++p){
    bool first=true;
    Row23 a,z;
    double max_rf=0,max_rg=0;
    for(const auto&r:in.rows){
      if(r.phase!=p)continue;
      if(first){a=r;first=false;}
      z=r;
...
int main(int argc,char**argv){
  ...
  int cfd=-1,sfd=-1;
  bool streaming=false,pipeline_started=false,aborted=false,imu_req=false,att_req=false;
  uint8_t sys=0,comp=0;
  std::vector<CameraBuffer>bufs;
  std::shared_ptr<Pipeline23>pipe;
  std::thread pipe_thread;
  Telemetry tel;

  try{
    VIO::VioParams vp(params);
    pipe=std::make_shared<Pipeline23>(vp);
    ...
    cv::setNumThreads(1);
    cv::namedWindow(kWindow23,cv::WINDOW_NORMAL);
    cv::resizeWindow(kWindow23,1440,940);

    std::vector<TimeSyncSample>sync;
    ClockMapping mapping;
    int64_t pending_sync=0,next_sync=monotonicNs(),next_hud=monotonicNs();
    uint32_t prev_seq=0;
    ...
    while(true){
            ...
            if(msg.msgid==MAVLINK_MSG_ID_TIMESYNC){
              ...
              if(have_att){
                ...
                while(!iq.empty()&&iq.front().us<=cur_att.us){
                  auto im=iq.front();
                  if(im.us>=prev_att.us&&ready){
                    process23(in,phase,im,interpR(prev_att,cur_att,im.us),*pipe);
                  }
                  iq.pop_front();
                }
              }else{
              ...
              if(ready&&!done){
                ...
                bool ok=std::abs(yy-kTargets23[phase])<=kYawTol23 &&
                        gn<=kGyroStill23 &&
                        std::abs(an-9.81)<=kAccTol23;
                if(ok)++stable;else stable=0;
                if(stable>=kStable23){
                  std::cout<<"[STEP] "<<kNames23[phase]<<" fixed.\n";
                  stable=0;
                  if(++phase>=4){
                    phase=3;
                    done=true;
                    ...
                    std::cout<<"[NEXT] target "<<kTargets23[phase]<<" deg.\n";
                  }
                }
              }
              ...
              ImuSample10 im{h.time_usec,h.xacc,-h.yacc,-h.zacc,h.xgyro,-h.ygyro,-h.zgyro};
              {
                std::lock_guard<std::mutex>q(tel.mutex);
              ...
              iq.push_back(im);
              while(iq.size()>300)iq.pop_front();
              if(mapping.valid&&rx-mapping.last_update_ns<=kMappingStaleNs){
                VIO::ImuAccGyr d;
                d<<im.ax,im.ay,im.az,im.gx,im.gy,im.gz;
                pipe->fillSingleImuQueue(VIO::ImuMeasurement(mapping.map((int64_t)h.time_usec*1000LL),d));
              }
            }
          }
        }
      ...
      if(pf[0].revents&POLLIN){
        for(;;){
          ...
          bool ok=true;
          if(have_prev){
            int64_t d=ts-prev_ts;
            ok=b.sequence==prev_seq+1U&&d>0&&d<=20000000LL;
          }
          prev_seq=b.sequence;
          prev_ts=ts;
          have_prev=true;
          ...
          if(ok&&due&&mapping.valid){
            ...
            if(!gray.empty()){
              last_gray=gray;
              pipe->fillLeftFrameQueue(std::make_unique<VIO::Frame>(fid++,ts,vp.camera_params_.at(0),gray.clone()));
              last_sel=ts;
            }
          }
      ...
      if(now>=next_hud){
        hud23(last_gray,tel,ready,done,phase,stable,in,ref);
      ...
      if(key==' '&&!ready){
        Backend23 bb;
        bool bv=pipe->latest(&bb),fv=false;
        Eigen::Matrix3d Rfc0=Eigen::Matrix3d::Identity();
        {
          std::lock_guard<std::mutex>q(tel.mutex);
          fv=tel.fc_valid;
          if(fv){
            ref=tel.fc_accum_yaw;
            Rfc0=fcRnedFlu(tel.fc_roll,tel.fc_pitch,tel.fc_yaw);
          }
        }
        if(bv&&fv&&have_att){
          in.world_map=bb.Rpred*Rfc0.transpose();
          in.Rgyro=bb.Rpred;
          in.fixed_ba=bb.BA;
          in.fixed_bg=bb.BG;
          in.pim_v0=bb.Vpred;

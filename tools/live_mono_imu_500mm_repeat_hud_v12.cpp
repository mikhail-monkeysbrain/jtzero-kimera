// JT-ZERO 500 mm x6 v12 — raw recording + TF-Luna DISTANCE_SENSOR.
...
constexpr const char* kV8CsvPath = "/home/vio/jtzero_500mm_v12_legs.csv";
constexpr const char* kV11CombinedPath = "/home/vio/jtzero_500mm_v12.csv";
constexpr const char* kV11CameraPath = "/home/vio/jtzero_500mm_v12_camera.csv";
constexpr const char* kV11MjpegPath = "/home/vio/jtzero_500mm_v12.mjpg";
constexpr const char* kV11AttitudePath = "/home/vio/jtzero_500mm_v12_attitude.csv";
constexpr const char* kV12RangePath = "/home/vio/jtzero_500mm_v12_range.csv";
constexpr const char* kV8WindowName = "JT-ZERO 500 мм x6 v12 — startup-gated";
...
void saveBackendTraceV8(const std::vector<VioState>& states,
  ...
  const char* path = "/home/vio/jtzero_500mm_v12_backend.csv";
...
int main(int argc,char**argv){
  ...
  try{
    ...
    if(!raw_combined||!raw_camera||!raw_mjpg||!raw_attitude||!raw_range)
      throw std::runtime_error("Cannot create v11 raw-recording files");
    ...
    while(leg<kLegCount){
      ...
      if(pf[1].revents&POLLIN){uint8_t b[8192];for(;;){ssize_t n=read(serial_fd,b,sizeof(b));if(n==-1&&(errno==EAGAIN||errno==EWOULDBLOCK))break;if(n<=0)break;for(ssize_t i=0;i<n;++i){
        ...
        if(msg.msgid==MAVLINK_MSG_ID_TIMESYNC){mavlink_timesync_t ts{};mavlink_msg_timesync_decode(&msg,&ts);if(ts.tc1!=0&&pending&&ts.ts1==pending){TimeSyncSample s;s.t0_rpi_ns=pending;s.t1_rpi_ns=recv;s.fc_ns=ts.tc1;s.rtt_ns=recv-pending;s.rpi_mid_ns=pending+s.rtt_ns/2;s.good=s.rtt_ns>0&&nsToMs(s.rtt_ns)<=kMaxTimesyncRttMs;sync.push_back(s);pending=0;mapping=estimateClockMapping(sync);}}
              ...
              ++raw_imu_rows;
              const bool moving=(phase==CapturePhase::MOVING);const Eigen::Vector3d w=imu_correction.correctGyro(h.time_usec,acc,gyro,!moving);VIO::ImuAccGyr data;data<<acc.x(),acc.y(),acc.z(),w.x(),w.y(),w.z();pipeline->fillSingleImuQueue(VIO::ImuMeasurement(mapped_imu_ns,data));++imu_fed;}
      ...
      if(pf[0].revents&POLLIN){for(;;){v4l2_buffer b{};b.type=V4L2_BUF_TYPE_VIDEO_CAPTURE;b.memory=V4L2_MEMORY_MMAP;if(xioctl(camera_fd,VIDIOC_DQBUF,&b)==-1){if(errno==EAGAIN)break;fail("DQBUF");}++raw;const int64_t raw_ts_ns=timevalToNs(b.timestamp);const int64_t corrected=jtzero::timesync::correctCameraTimestampNs(raw_ts_ns);
        uint64_t this_offset=raw_mjpg_offset;
        if(vio_feed_enabled&&b.bytesused>0){
          raw_mjpg.write(reinterpret_cast<const char*>(buffers[b.index].start),(std::streamsize)b.bytesused);
          raw_mjpg_offset+=b.bytesused;
          ++raw_camera_rows;
        }
    ...
    std::cout<<"RAW IMU CSV: "<<kV11CombinedPath<<" rows="<<raw_imu_rows<<"\n"
             ...
             <<"LEGS CSV: "<<kV8CsvPath<<"\n"
             <<"BACKEND CSV: /home/vio/jtzero_500mm_v12_backend.csv\n";

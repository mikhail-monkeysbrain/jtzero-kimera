...
void drawRepeatHud(const cv::Mat&gray,const HudPipeline&pipeline,int leg,const std::string&dir,bool have_start){
  ...
  txt(canvas,"JT-ZERO 300 mm x6",{28,52},1.12,{245,245,245},3);txt(canvas,"IMU: FLU + ZXY + gravity feedback",{28,84},.55,{190,190,190},1);
  char buf[192];std::snprintf(buf,sizeof(buf),"LEG %d / %d",leg+1,kLegCount);txt(panel,buf,{18,55},.92,{245,245,245},3);txt(panel,dir,{18,105},1.05,{0,230,255},3);
  if(!have_start){txt(panel,"Keep still at start mark",{18,180},.67,{220,220,220},2);txt(panel,"SPACE = capture START",{18,230},.70,{90,220,90},2);}else{txt(panel,"Move exactly 300 mm",{18,180},.67,{220,220,220},2);txt(panel,"Stop at mechanical mark",{18,225},.62,{220,220,220},2);txt(panel,"SPACE = capture END",{18,275},.70,{90,220,90},2);}
  VioState s;if(pipeline.latest(&s)){std::snprintf(buf,sizeof(buf),"P [%.3f %.3f %.3f] m",s.px,s.py,s.pz);txt(panel,buf,{18,390},.58,{220,220,220},1);const double v=std::sqrt(s.vx*s.vx+s.vy*s.vy+s.vz*s.vz)*1000.0;std::snprintf(buf,sizeof(buf),"|V| %.1f mm/s",v);txt(panel,buf,{18,430},.58,{220,220,220},1);}
  txt(panel,"Physical marks are truth",{18,720},.58,{185,185,185},1);txt(panel,"Q / ESC = abort",{18,765},.58,{215,215,215},1);panel.copyTo(canvas(cv::Rect(900,0,380,900)));cv::imshow(kRepeatWindow,canvas);
}

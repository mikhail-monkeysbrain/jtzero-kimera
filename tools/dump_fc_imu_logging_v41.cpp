// JT-ZERO v41: чтение параметров логирования IMU ArduPilot.
// Только чтение. Никакие параметры полётного контроллера не изменяются.

#define main jtzero_dump_unused_main_v41
#include "camera_imu_extrinsics_logger.cpp"
#undef main

namespace {

bool wanted41(const std::string& n) {
  if (n == "LOG_BITMASK") return true;
  if (n.rfind("INS_RAW_LOG", 0) == 0) return true;
  if (n.rfind("INS_LOG_BAT", 0) == 0) return true;
  if (n.rfind("INS_LOG", 0) == 0) return true;
  if (n.rfind("INS_FAST", 0) == 0) return true;
...
void requestList41(int fd, uint8_t sys, uint8_t comp) {
  mavlink_message_t m{}; uint8_t b[MAVLINK_MAX_PACKET_LEN];
  mavlink_msg_param_request_list_pack(COMPANION_SYSID, COMPANION_COMPID,
                                      &m, sys, comp);
  const uint16_t n = mavlink_msg_to_send_buffer(b, &m);
  if (write(fd, b, n) != (ssize_t)n) fail("write PARAM_REQUEST_LIST");
}

std::string pname41(const mavlink_param_value_t& p) {
  char b[17]{};
...
int main() {
  ...
  try {
    ...
    std::cout << "[MAV] FC sysid=" << (int)sys
              << " compid=" << (int)comp << "\n";
    requestList41(fd, sys, comp);
    ...
    while (monotonicNs() < deadline) {
      ...
      for (ssize_t i = 0; i < n; ++i) {
        ...
        last_rx = monotonicNs();
        expected = pv.param_count;
        const std::string name = pname41(pv);
        if (wanted41(name)) vals[name] = pv.param_value;
      }
    }

    std::cout << "\n================ ПАРАМЕТРЫ ЛОГИРОВАНИЯ IMU v41 ================\n";
    std::cout << "Всего параметров FC: " << expected << "\n";
    std::cout << "Выбрано параметров: " << vals.size() << "\n\n";
    if (vals.empty()) {
      std::cout << "Подходящие параметры не найдены в этой прошивке.\n";
    } else {
      for (const auto& kv : vals) {
        std::cout << std::left << std::setw(24) << kv.first
                  << " = " << std::setprecision(10) << kv.second << "\n";
      }
    }

...
## Постоянное правило интерфейса — 2026-09-08

**ЗАПРЕЩЕНО делать операторский интерфейс тестовых программ JT-Zero на английском языке.**

Все надписи, инструкции, состояния, предупреждения, подсказки клавиш и сообщения оператору в GUI/HUD должны быть **на русском языке**. Английский допустим только для технических идентификаторов, имён файлов, полей CSV, стандартных сокращений и терминов, которые не являются операторскими инструкциями.
...
## 2026-09-08 — исправлена сборка v38

Первый запуск статического теста v38 не состоялся: компиляция остановилась из-за конфликтов имён `Att`, `kWindow`, `kCsv` с символами из включённого `live_mono_imu_ypr_dashboard_v8.cpp`.

Исправление:
- `Att` -> `Att38`;
- `kWindow` -> `kWindow38`;
- `kCsv` -> `kCsv38`;
- ссылки в `Row`, HUD и main обновлены.
...
## 2026-09-09 — пользовательский режим непрерывных тестов

Пользователь зафиксировал обязательное правило рабочего процесса: если в его сообщении явно не указано иное, каждый следующий ответ ассистента должен содержать **следующий новый тест и готовые инструкции к его выполнению**. Не останавливаться для запроса подтверждения между тестами.

Ограничения сохраняются: перед каждым новым тестом проверять журнал/репозиторий на повтор; не повторять закрытый тест без существенного отличия; после результата сразу анализировать его и выдавать следующий диагностически обоснованный тест. Если следующий шаг не требует физического движения, это всё равно оформляется как отдельный диагностический тест с готовой командой.

Текущий переход после V42: новый физический A→B пока не нужен. Сначала использовать уже сохранённый архив V42 для независимого разбора расхождения CAMERA-ONLY 566.61 мм против KIMERA 491.34 мм и ложного ΔZ +51.02 мм, одновременно не смешивая camera-only и fusion как независимые доказательства одной причины.

**Статус:** РАБОЧИЙ ПРОЦЕСС ОБНОВЛЁН — ответы идут непрерывной последовательностью тестов без ожидания подтверждения пользователя.


## 2026-09-09 — V42 metric reconciliation prepared

После V42 архив содержит backend/frontend/IMU данные, но не содержит отдельный per-frame CAMERA-ONLY trace и direct-Luna CSV: прямой TF-Luna использовался runtime-потоком, а raw range CSV для FC остался пустым. Поэтому полный camera-only trajectory из архива восстановить нельзя без изображений.

Перед новым физическим run выбран более дешёвый discriminator: количественно проверить, может ли выбранная высота камеры объяснить CAMERA-ONLY=566.61 мм при физической истине 500 мм и direct TF-Luna=180 мм.

Добавлен `tools/analyze_v42_metric_reconciliation.py`. Он использует линейную зависимость metric camera-only масштаба от высоты в текущем V41/V42 алгоритме и вычисляет: эффект camera offset, residual при h=TF-Luna, и высоту, которая потребовалась бы для ровно 500 мм.

Это не повторяет V41 range forensic: V41 проверял тракт дальности; V42 reconciliation проверяет достаточность height-scale объяснения уже после перехода на direct TF-Luna.

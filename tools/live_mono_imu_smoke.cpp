...
struct ClockMapping {
  ...
  int64_t map(int64_t fc_ns) const {
    return static_cast<int64_t>(
        std::llround(rpi_ref_ns + a * static_cast<long double>(fc_ns - fc_ref_ns)));
  }
};
...
void setCameraControl(int fd, uint32_t id, int32_t value, const char* name) {
  ...
  if (xioctl(fd, VIDIOC_S_CTRL, &c) == -1) {
    std::cerr << "[WARN] " << name << " failed: " << std::strerror(errno) << "\n";
  }
}
...
void configureCamera(int fd) {
  ...
  setCameraControl(fd, V4L2_CID_EXPOSURE_AUTO, V4L2_EXPOSURE_MANUAL, "auto_exposure");
  setCameraControl(fd, V4L2_CID_EXPOSURE_AUTO_PRIORITY, 0, "dynamic_framerate");
  setCameraControl(fd, V4L2_CID_EXPOSURE_ABSOLUTE, kExposureAbsolute, "exposure_absolute");
  setCameraControl(fd, V4L2_CID_GAIN, kGain, "gain");
  setCameraControl(fd, V4L2_CID_AUTO_WHITE_BALANCE, 0, "white_balance_automatic");
  setCameraControl(fd, V4L2_CID_POWER_LINE_FREQUENCY,
                   V4L2_CID_POWER_LINE_FREQUENCY_DISABLED, "power_line_frequency");
  setCameraControl(fd, V4L2_CID_BACKLIGHT_COMPENSATION, 0, "backlight_compensation");
}
...
void discardWarmup(int fd, int count) {
  for (int n = 0; n < count;) {
...
void serialWriteAll(int fd, const uint8_t* data, size_t size) {
  for (size_t p = 0; p < size;) {
    const ssize_t n = write(fd, data + p, size - p);
    if (n > 0) {
...
void requestRate(int fd, uint8_t sys, uint8_t comp, uint32_t msgid, int hz) {
  ...
  mavlink_msg_command_long_pack(
      kCompanionSysId, kCompanionCompId, &m, sys, comp,
...
void sendTimesync(int fd, int64_t t0, uint8_t sys, uint8_t comp) {
  ...
  mavlink_msg_timesync_pack(
      kCompanionSysId, kCompanionCompId, &m, 0, t0, sys, comp);
  sendMsg(fd, m);
}
...
class LiveMonoImuPipeline final : public VIO::MonoImuPipeline {
 public:
  explicit LiveMonoImuPipeline(const VIO::VioParams& params)
  ...
  void installBackendCallback() {
    registerBackendOutputCallback(
        [this](const std::shared_ptr<VIO::BackendOutput>& out) {
          ...
          const auto t = state.pose_.translation();
          const auto rpy = state.pose_.rotation().rpy();
          const auto& v = state.velocity_;
          std::cout << std::fixed << std::setprecision(4)
                    << "[VIO] kf=" << out->cur_kf_id_
                    << " t=" << state.timestamp_
                    << " P=[" << t.x() << "," << t.y() << "," << t.z() << "]"
                    << " V=[" << v.x() << "," << v.y() << "," << v.z() << "]"
                    << " RPYdeg=[" << rpy.x() * 180.0 / M_PI << ","
                    << rpy.y() * 180.0 / M_PI << ","
                    << rpy.z() * 180.0 / M_PI << "]\n";
        });
  }

  size_t backendOutputs() const { return backend_outputs_.load(); }

 private:
  std::atomic<size_t> backend_outputs_{0};
};
...
int main(int argc, char** argv) {
  ...
  const std::string params =
      argc > 1 ? argv[1] : "params/JTZeroMono";
  ...
  try {
    ...
    auto pipeline = std::make_shared<LiveMonoImuPipeline>(vio_params);
    pipeline->installBackendCallback();

    auto pipeline_thread =
        std::async(std::launch::async, [pipeline]() { return pipeline->spin(); });
    ...
    streaming = true;
    discardWarmup(camera_fd, kWarmupFrames);

    std::cout << "[LIVE] warming TIMESYNC mapping before feeding Kimera...\n";

    std::vector<TimeSyncSample> timesync;
    timesync.reserve(1000);
    ...
    const int64_t stop_ns =
        start_ns + static_cast<int64_t>(kRunSeconds * 1e9);
    ...
    while (monotonicNs() < stop_ns) {
        ...
        for (;;) {
          ...
          for (ssize_t i = 0; i < n; ++i) {
            if (!mavlink_parse_char(
                    MAVLINK_COMM_0, bytes[i], &mav_msg, &mav_status)) {
              continue;
            }
            ...
            if (mav_msg.msgid == MAVLINK_MSG_ID_TIMESYNC) {
              ...
              if (ts.tc1 != 0 && pending_timesync != 0 &&
                  ts.ts1 == pending_timesync) {
                TimeSyncSample s;
                ...
                s.good = s.rtt_ns > 0 &&
                         nsToMs(s.rtt_ns) <= kMaxTimesyncRttMs;
              ...
              if (!mapping.valid ||
                  recv_ns - mapping.last_update_ns > kMappingStaleNs) {
                ++imu_skipped_mapping;
                continue;
              }
              ...
              pipeline->fillSingleImuQueue(
                  VIO::ImuMeasurement(mapped_ns, data));
              ++imu_fed;
            }
          }
        }
      ...
      if (pending_timesync != 0 &&
          monotonicNs() - pending_timesync > 20000000LL) {
        pending_timesync = 0;
      }
      ...
      if (pf[0].revents & POLLIN) {
        for (;;) {
          ...
          const int64_t corrected_ns =
              jtzero::timesync::correctCameraTimestampNs(source_ns);

          bool raw_ok = true;
          if (have_previous_raw) {
            const int64_t dt = corrected_ns - previous_corrected_ns;
            raw_ok = (b.sequence == previous_seq + 1U) &&
                     dt > 0 &&
                     dt <= 20000000LL;
          ...
          if (raw_ok && selector_due && mapping.valid) {
            ...
            if (!gray.empty()) {
              ++decoded_frames;
              pipeline->fillLeftFrameQueue(
                  std::make_unique<VIO::Frame>(
                      frame_id++, corrected_ns,
                      vio_params.camera_params_.at(0), gray.clone()));
    ...
    std::cout << "[LIVE] stopping input and draining pipeline...\n";
    ...
    std::cout << "\n============================================================\n"
              << "JT-ZERO LIVE MONO+IMU SMOKE RESULT\n"
              << "============================================================\n"
              << "raw camera frames:      " << raw_frames << "\n"
              << "rejected raw pairs:     " << rejected_raw_pairs << "\n"
              << "selected camera frames: " << selected_frames << "\n"
              ...
              << mapping.drift_ppm << "\n"
              << "backend outputs:        " << pipeline->backendOutputs() << "\n";

    const bool pass =
        mapping.valid &&
        selected_frames >= 100 &&
        imu_fed >= 500 &&
        pipeline->backendOutputs() > 0;

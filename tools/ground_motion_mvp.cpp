...
struct FcHistory {
  ...
  static void requestRate(int fd,uint8_t sys,uint8_t comp,uint32_t msgid,int hz){mavlink_message_t m{};mavlink_msg_command_long_pack(self_sys,self_comp,&m,sys,comp,MAV_CMD_SET_MESSAGE_INTERVAL,0,msgid,1000000.0f/hz,0,0,0,0,0);uint8_t b[MAVLINK_MAX_PACKET_LEN];auto n=mavlink_msg_to_send_buffer(b,&m);writeAll(fd,b,n);}
  bool selectSourceSet(int set){if(fd<0||target_sys==0||set<1||set>3)return false;mavlink_message_t m{};mavlink_msg_command_long_pack(self_sys,self_comp,&m,target_sys,target_comp,kMavCmdSetEkfSourceSet,0,(float)set,0,0,0,0,0,0);uint8_t b[MAVLINK_MAX_PACKET_LEN];auto n=mavlink_msg_to_send_buffer(b,&m);writeAll(fd,b,n);std::cerr<<"\nEKF SOURCE SET: запрос SRC"<<set<<" отправлен target="<<(int)target_sys<<"/"<<(int)target_comp<<" from="<<(int)self_sys<<"/"<<(int)self_comp<<"\n";return true;}
  void start(const std::string&dev){fd=::open(dev.c_str(),O_RDWR|O_NOCTTY|O_NONBLOCK);if(fd<0)fail("open FC");termios t{};if(tcgetattr(fd,&t)<0)fail("FC tcgetattr");cfmakeraw(&t);cfsetispeed(&t,B460800);cfsetospeed(&t,B460800);t.c_cflag|=CLOCAL|CREAD;t.c_cflag&=~CRTSCTS;t.c_cflag&=~PARENB;t.c_cflag&=~CSTOPB;t.c_cflag&=~CSIZE;t.c_cflag|=CS8;if(tcsetattr(fd,TCSANOW,&t)<0)fail("FC tcsetattr");tcflush(fd,TCIFLUSH);th=std::thread([this]{mavlink_status_t st{};mavlink_message_t m{};uint8_t sys=0,comp=0,buf[4096];int64_t deadline=monoNs()+10000000000LL;while(g_running&&!sys&&monoNs()<deadline){pollfd p{fd,POLLIN,0};if(poll(&p,1,100)<=0)continue;ssize_t n=read(fd,buf,sizeof(buf));if(n<=0)continue;for(ssize_t i=0;i<n;i++)if(mavlink_parse_char(MAVLINK_COMM_0,buf[i],&m,&st)&&m.msgid==MAVLINK_MSG_ID_HEARTBEAT){mavlink_heartbeat_t hb{};mavlink_msg_heartbeat_decode(&m,&hb);if(hb.autopilot==MAV_AUTOPILOT_ARDUPILOTMEGA){sys=m.sysid;comp=m.compid;break;}}}if(!sys){std::cerr<<"FC: ArduPilot HEARTBEAT timeout\n";g_running=false;return;}target_sys=sys;target_comp=comp;std::cerr<<"FC: ArduPilot heartbeat sys="<<(int)sys<<" comp="<<(int)comp<<"\n";requestRate(fd,sys,comp,MAVLINK_MSG_ID_ATTITUDE,50);while(g_running){pollfd p{fd,POLLIN,0};if(poll(&p,1,50)<=0)continue;for(;;){ssize_t n=read(fd,buf,sizeof(buf));if(n<0&&(errno==EAGAIN||errno==EWOULDBLOCK))break;if(n<=0)break;for(ssize_t i=0;i<n;i++)if(mavlink_parse_char(MAVLINK_COMM_0,buf[i],&m,&st)){
          if(m.msgid==MAVLINK_MSG_ID_ATTITUDE){mavlink_attitude_t a{};mavlink_msg_attitude_decode(&m,&a);Attitude x{a.roll,a.pitch,a.yaw,monoNs(),true};std::lock_guard<std::mutex>l(mu);hist.push_back(x);int64_t keep=x.recv_ns-3000000000LL;while(hist.size()>2&&hist.front().recv_ns<keep)hist.pop_front();}
          else if(m.msgid==MAVLINK_MSG_ID_COMMAND_ACK){mavlink_command_ack_t a{};mavlink_msg_command_ack_decode(&m,&a);if(a.command==kMavCmdSetEkfSourceSet)std::cerr<<"\nEKF SOURCE SET ACK: result="<<(int)a.result<<" from sys="<<(int)m.sysid<<" comp="<<(int)m.compid<<"\n";}
        }}}});}
...
int main(int argc,char**argv){
 ...
 if(hl&&ha){auto R=attitudeFluToNwu(att);down=-(R*cv::Vec3d(0,0,-1))[2];h=lm*down-offset+(R*lever_b)[2];}bool sensors=hl&&ha&&down>0.20&&h>0.05&&lage<200&&att_age<30;
 ...
 if(frame%100==0)std::cerr<<"GM frame="<<frame<<" valid="<<(valid?1:0)<<" VISION_SPEED_ESTIMATE sent="<<mav_sent<<" skipped="<<mav_skipped<<" DISTANCE_SENSOR sent="<<range_sent<<" skipped="<<range_skipped<<"\r"<<std::flush;

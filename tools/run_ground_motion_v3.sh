...
BIN="/tmp/jtzero_ground_motion_v3"
...
OUT="${JTZERO_GM_OUT:-$HOME/jtzero_runs/${STAMP}_GROUND_MOTION_V3.csv}"

mkdir -p "$(dirname "$OUT")"

if [[ ! -x "$BIN" || "$ROOT/tools/ground_motion_live_v3.cpp" -nt "$BIN" ]]; then
  bash "$ROOT/tools/build_ground_motion_v3.sh" "$BIN"
fi

echo "======================================================================"
echo "JT-ZERO GROUND MOTION V3"
echo "Камера: $CAMERA"
echo "TF-Luna: $LUNA"
echo "FC: $FC"
echo "CSV: $OUT"

...
void ru(cv::Mat& img, const std::string& s, cv::Point p, int size,
  ...
  // OpenCV is built with Qt on the JT-ZERO workstation; cv::addText uses Qt
  // and therefore renders UTF-8/Cyrillic correctly, unlike cv::putText.
...
void drawRepeatHud(const cv::Mat& gray, const HudPipeline& pipeline,
  ...
  ru(canvas, "JT-ZERO — калибровка 300 мм × 6", {28,52}, 28, {245,245,245}, cv::QT_FONT_BOLD);
  ...
  const bool fresh = have_vio && now - s.callback_wall_ns <= kStateStaleNs;
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    while(leg<kLegCount){
      ...
      const bool keyboard_capture=(key==32||key==13||key==10);
      const bool capture=keyboard_capture||gMouseCapture;
      gMouseCapture=false;

      if(capture){
        VioState s;
        if(!pipeline->latest(&s) || monotonicNs()-s.callback_wall_ns>kStateStaleNs){

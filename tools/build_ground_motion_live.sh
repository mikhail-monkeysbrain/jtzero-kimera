...
OUT="${1:-/tmp/jtzero_ground_motion_live}"
MAVLINK_INC=""
for d in "$KIMERA_ROOT/third_party/mavlink" "$KIMERA_ROOT/third_party/mavlink/include/mavlink/v2.0" "/usr/local/include/mavlink/v2.0"; do
  [[ -f "$d/common/mavlink.h" ]] && MAVLINK_INC="-I$d" && break
done
[[ -n "$MAVLINK_INC" ]] || { echo "ОШИБКА: common/mavlink.h не найден"; exit 2; }
g++ -std=c++17 -O2 -pthread $(pkg-config --cflags opencv4) $MAVLINK_INC "$ROOT/tools/ground_motion_live.cpp" -o "$OUT" $(pkg-config --libs opencv4) -lpthread
echo "ГОТОВО: $OUT"

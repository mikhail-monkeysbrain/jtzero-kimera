...
constexpr const char* kCsvPath = "/home/vio/jtzero_live_standstill.csv";
...
constexpr double kDefaultRunSeconds = 120.0;
...
class StandstillPipeline final : public VIO::MonoImuPipeline {
 public:
  explicit StandstillPipeline(const VIO::VioParams& params)
...
void printStandstillMetrics(const std::vector<VioState>& states) {
  if (states.empty()) {
    std::cout << "standstill states:     0\n";
    return;
  }

  const auto& first = states.front();
  const auto& last = states.back();
  double sum_p2 = 0, max_p = 0;
  double sum_v2 = 0, max_v = 0;

  for (const auto& s : states) {
    const double dx = s.px - first.px;
    const double dy = s.py - first.py;
    const double dz = s.pz - first.pz;
    const double pn = std::sqrt(dx*dx + dy*dy + dz*dz);
    const double vn = std::sqrt(s.vx*s.vx + s.vy*s.vy + s.vz*s.vz);
    sum_p2 += pn * pn;
    sum_v2 += vn * vn;
    max_p = std::max(max_p, pn);
    max_v = std::max(max_v, vn);
  }

  const double fdx = last.px - first.px;
  const double fdy = last.py - first.py;
  const double fdz = last.pz - first.pz;
  const double final_p = std::sqrt(fdx*fdx + fdy*fdy + fdz*fdz);
  const double rms_p = std::sqrt(sum_p2 / states.size());
  const double rms_v = std::sqrt(sum_v2 / states.size());
  const double dt_s = (last.timestamp_ns - first.timestamp_ns) / 1e9;

  std::cout << std::fixed << std::setprecision(6)
            << "standstill states:     " << states.size() << "\n"
            << "backend duration:      " << dt_s << " s\n"
            << "final dP:              [" << fdx << "," << fdy << "," << fdz << "] m\n"
            << "final |dP|:            " << final_p * 1000.0 << " mm\n"
            << "position RMS origin:   " << rms_p * 1000.0 << " mm\n"
            << "position max origin:   " << max_p * 1000.0 << " mm\n"
            << "velocity RMS:          " << rms_v * 1000.0 << " mm/s\n"
            << "velocity max:          " << max_v * 1000.0 << " mm/s\n"
            << "roll drift:            " << wrapDeg(last.roll_deg-first.roll_deg) << " deg\n"
            << "pitch drift:           " << wrapDeg(last.pitch_deg-first.pitch_deg) << " deg\n"
            << "yaw drift:             " << wrapDeg(last.yaw_deg-first.yaw_deg) << " deg\n";
}
...
int main(int argc, char** argv) {
  ...
  double run_seconds = kDefaultRunSeconds;
  if (argc > 2) run_seconds = std::stod(argv[2]);
  if (run_seconds < 10.0) throw std::runtime_error("run_seconds must be >= 10");
  ...
  try {
    ...
    std::cout << "[STANDSTILL] Keep the complete rig motionless for "
              << run_seconds << " seconds.\n";
    std::cout << "[STANDSTILL] Visualizer disabled. Backend CSV: " << kCsvPath << "\n";

    std::vector<TimeSyncSample> timesync;
    timesync.reserve(static_cast<size_t>(run_seconds * 12));
    ...
    const int64_t stop_ns = start_ns + static_cast<int64_t>(run_seconds * 1e9);
    ...
    while (monotonicNs() < stop_ns) {
      ...
      if (pending_timesync && monotonicNs() - pending_timesync > 20000000LL)
        pending_timesync = 0;
    ...
    std::cout << "[STANDSTILL] stopping and draining...\n";
    ...
    std::cout << "\n============================================================\n"
              << "JT-ZERO LIVE MONO+IMU STANDSTILL RESULT\n"
              << "============================================================\n"
              << "requested duration:    " << run_seconds << " s\n"
              ...
              << mapping.drift_ppm << "\n";
    printStandstillMetrics(states);
    std::cout << "CSV:                    " << kCsvPath << "\n";

    const bool pass = mapping.valid && selected_frames >= run_seconds * 20.0 &&
                      imu_fed >= run_seconds * 150.0 && states.size() >= 100;
    std::cout << "PIPELINE RESULT:        " << (pass ? "PASS" : "FAIL") << "\n";
    std::cout << "DRIFT QUALIFICATION:    REPORT ONLY (thresholds not frozen yet)\n";
    return pass ? 0 : 1;

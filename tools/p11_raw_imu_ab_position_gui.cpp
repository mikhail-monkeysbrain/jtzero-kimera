...
static void txt(cv::Mat& i, const std::string& s, int y, double z=.75, int th=2){
  const int font_px=std::max(18,static_cast<int>(std::lround(30.0*z)));
  cv::addText(i,s,{30,y},"DejaVu Sans",font_px,{235,235,235},th,cv::LINE_AA,false);
}
int main(){const char* L[]={"A1","B1","A2","B2","A3","B3","A4"};const char* P[]={"A","B","A","B","A","B","A"};const double SEC=10;
 ...
 cv::namedWindow("P11: проверка IMU в точках A/B",cv::WINDOW_NORMAL);cv::resizeWindow("P11: проверка IMU в точках A/B",1000,650);int s=0;bool rec=false,finished=false;int64_t t0=0;double ax=0,ay=0,az=0,gx=0,gy=0,gz=0,tp=0;uint64_t tu=0;long count=0;
 while(!finished){pollfd q{f,POLLIN,0};poll(&q,1,5);uint8_t b[4096];int n=read(f,b,sizeof(b));for(int j=0;j<n;j++)if(mavlink_parse_char(MAVLINK_COMM_0,b[j],&m,&st)&&m.msgid==MAVLINK_MSG_ID_HIGHRES_IMU){mavlink_highres_imu_t x{};mavlink_msg_highres_imu_decode(&m,&x);ax=x.xacc;ay=x.yacc;az=x.zacc;gx=x.xgyro;gy=x.ygyro;gz=x.zgyro;tp=x.temperature;tu=x.time_usec;count++;double an=sqrt(ax*ax+ay*ay+az*az);o<<ns()<<','<<tu<<','<<L[s]<<','<<P[s]<<','<<(rec?1:0)<<','<<std::setprecision(9)<<ax<<','<<ay<<','<<az<<','<<ax<<','<<-ay<<','<<-az<<','<<an<<','<<gx<<','<<gy<<','<<gz<<','<<gx<<','<<-gy<<','<<-gz<<','<<tp<<"\n";}
  ...
  cv::Mat im(650,1000,CV_8UC3,cv::Scalar(22,22,22));txt(im,"P11: проверка сырого IMU в точках A/B",55,.95);txt(im,"Этап "+std::to_string(s+1)+"/7   "+L[s]+"   ТОЧКА "+P[s],115,1.0);
  if(rec){txt(im,"НЕ ТРОГАЙТЕ БПЛА",190,1.0,3);txt(im,"Запись неподвижного участка: осталось "+std::to_string(std::max(0,(int)ceil(SEC-el)))+" с",240,.9);}
  else{txt(im,s==0?"Установите БПЛА в точку A и уберите руки.":"Перемещение закончено. Не касайтесь БПЛА.",190,.82);txt(im,"Когда БПЛА полностью неподвижен, нажмите ПРОБЕЛ.",240,.82);}
  std::ostringstream z;z<<std::fixed<<std::setprecision(4)<<"FRD ускорение=["<<ax<<", "<<ay<<", "<<az<<"]  |a|="<<sqrt(ax*ax+ay*ay+az*az)<<"  T="<<tp;txt(im,z.str(),330,.65,1);
  txt(im,"Протокол: A1 -> B1 -> A2 -> B2 -> A3 -> B3 -> A4",400,.7);if(!rec&&s<6)txt(im,std::string("После записи: переместите ")+P[s]+" -> "+P[s+1]+", затем уберите руки.",450,.65,1);
  txt(im,"ПРОБЕЛ = записать 10 с покоя     ESC = прервать тест",570,.68);cv::imshow("P11: проверка IMU в точках A/B",im);int k=cv::waitKey(1)&255;if(k==27){e<<ns()<<",ABORT,"<<L[s]<<','<<P[s]<<"\n";break;}if(k==' '&&!rec){rec=true;t0=ns();e<<t0<<",PLATEAU_START,"<<L[s]<<','<<P[s]<<"\n";e.flush();}}

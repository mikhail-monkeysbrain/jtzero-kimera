...
SRC="${ROOT}/tools/live_mono_imu_500mm_v42_direct_luna_camera_vs_fusion.cpp"
BIN="/tmp/live_mono_imu_500mm_v42_direct_luna_camera_vs_fusion"
STAMP=$(date +%Y%m%d_%H%M%S)
OUT="${RUNS}/${STAMP}_v42_DIRECT_LUNA_CAMERA_VS_FUSION"
...
export JTZERO_V41_CAMERA_HEIGHT_OFFSET_M="${JTZERO_V42_CAMERA_HEIGHT_OFFSET_M:-0.005}"
...
echo "======================================================================"
echo "V42 — ОДИН КОНТРОЛЬНЫЙ ПРОХОД A→B / 500 мм"
echo "ОБЩИЙ ПРОГРЕСС: ШАГ 4 ИЗ 5"
...
echo "3) Убедиться, что V42-PREFLIGHT показывает валидную TF-Luna."
...
echo
echo "ПОСЛЕ ЭТОГО ДОПОЛНИТЕЛЬНЫХ ПРОХОДОВ НЕ БУДЕТ."
echo "======================================================================"
...
if [[ ! -x "${BIN}" || "${SRC}" -nt "${BIN}" ||       "${ROOT}/tools/v25_parts/live_mono_imu_500mm_repeat_hud_v25_part01.inc" -nt "${BIN}" ||       "${ROOT}/tools/v18_parts/live_mono_imu_500mm_repeat_hud_v18_part07b.inc" -nt "${BIN}" ||       "${ROOT}/tools/v25_parts/live_mono_imu_500mm_repeat_hud_v25_part08b.inc" -nt "${BIN}" ]]; then
  echo "[V42] Исходники изменились — собираю бинарник ОДИН РАЗ..."
  bash "${ROOT}/tools/build_v25.sh" "${BIN}"
else
  echo "[V42] Использую уже собранный бинарник: ${BIN}"
fi
...
read -r -p "ENTER — запустить единственный V42 проход: " _
set +e
bash "${ROOT}/tools/run_v25_auto_camera.sh" CONTROLLED_AB4
RC=$?
set -e

A=$(bash "${ROOT}/tools/archive_v25_run.sh" "V42_DIRECT_LUNA_CAMERA_VS_FUSION" "${PARAMS}")
mkdir -p "${OUT}"
cp -a "${A}/." "${OUT}/"
{
  echo "test=v42_direct_luna_camera_vs_fusion"
...
} > "${OUT}/V42_METADATA.txt"

echo
echo "======================================================================"
echo "V42 ЗАВЕРШЁН — 1 ИЗ 1."

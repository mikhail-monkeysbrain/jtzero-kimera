...
LABEL="${1:-CONTROLLED_AB4_SLOW_10S}"
...
export JTZERO_V26_TARGET_SEC=10.0
export JTZERO_V26_TOLERANCE_SEC=1.0
export JTZERO_V25_MODE_DESC="V26 CONTROLLED_AB4 SLOW: four measured A->B passes; target 10.0 s; accepted 9.0..11.0 s"

rm -f "$HOME"/jtzero_500mm_v26_[1-4]AB.csv

echo "[V26-SLOW10] params=$PARAMS"
echo "[V26-SLOW10] mode=CONTROLLED_AB4"
echo "[V26-SLOW10] fixed target for ALL passes: 10.0 s; accepted window: 9.0..11.0 s"
echo "[V26-SLOW10] rebuilding binary..."
...
echo
echo "[V26-SLOW10] run exit code=$rc"
echo "[V26-SLOW10] archiving outputs..."
archive_path="$(bash "$ROOT/tools/archive_v25_run.sh" "$LABEL" "$PARAMS")"
echo "[V26-SLOW10] archive=$archive_path"

for f in "$HOME"/jtzero_500mm_v26_[1-4]AB.csv; do
  [[ -e "$f" ]] && echo "[V26-SLOW10] comparison CSV: $f"
done

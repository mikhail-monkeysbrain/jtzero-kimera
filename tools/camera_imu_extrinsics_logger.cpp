...
constexpr const char* PREVIEW_WINDOW = "JT-Zero Camera + IMU Calibration";
...
constexpr int PREVIEW_DIVIDER = 4;  // 120 FPS source -> <=30 FPS preview.
...
struct ClockMapping {
    ...
    int64_t map(int64_t fc_ns) const {
        const long double d = static_cast<long double>(fc_ns - fc_ref_ns);
        return static_cast<int64_t>(std::llround(rpi_ref_ns + a * d));
    }
};
    ...
    std::string s =
        ((flags & V4L2_BUF_FLAG_TIMESTAMP_MASK) ==
         V4L2_BUF_FLAG_TIMESTAMP_MONOTONIC)
            ...
            : "other_clock";
    s += "|";
    s += ((flags & V4L2_BUF_FLAG_TSTAMP_SRC_MASK) ==
...
void drawReticleAndTimer(cv::Mat& frame, const PreviewPacket& packet) {
    const int cx = frame.cols / 2;
    const int cy = frame.rows / 2;
    const cv::Scalar white(255, 255, 255);
    const cv::Scalar black(0, 0, 0);

    // Black outline + white reticle keeps it visible on both light/dark ChArUco cells.
    for (int thickness : {5, 2}) {
        const cv::Scalar color = thickness == 5 ? black : white;
        cv::line(frame, {cx - 55, cy}, {cx - 8, cy}, color, thickness, cv::LINE_AA);
        cv::line(frame, {cx + 8, cy}, {cx + 55, cy}, color, thickness, cv::LINE_AA);
        cv::line(frame, {cx, cy - 55}, {cx, cy - 8}, color, thickness, cv::LINE_AA);
        cv::line(frame, {cx, cy + 8}, {cx, cy + 55}, color, thickness, cv::LINE_AA);
        cv::circle(frame, {cx, cy}, 22, color, thickness, cv::LINE_AA);
    }
    cv::circle(frame, {cx, cy}, 3, black, 7, cv::LINE_AA);
    cv::circle(frame, {cx, cy}, 3, white, 3, cv::LINE_AA);

    const double remaining = std::max(0.0, packet.duration_sec - packet.elapsed_sec);
    std::ostringstream timer;
    timer << std::fixed << std::setprecision(1)
          << "TIME " << packet.elapsed_sec << " / " << packet.duration_sec
          << " s   LEFT " << remaining << " s";

    cv::putText(frame, timer.str(), {16, 32}, cv::FONT_HERSHEY_SIMPLEX,
                0.65, black, 4, cv::LINE_AA);
    cv::putText(frame, timer.str(), {16, 32}, cv::FONT_HERSHEY_SIMPLEX,
                0.65, white, 1, cv::LINE_AA);

    const std::string hint = "Center board on reticle   Q/ESC = stop";
    cv::putText(frame, hint, {16, frame.rows - 18}, cv::FONT_HERSHEY_SIMPLEX,
                0.52, black, 4, cv::LINE_AA);
    cv::putText(frame, hint, {16, frame.rows - 18}, cv::FONT_HERSHEY_SIMPLEX,
                0.52, white, 1, cv::LINE_AA);
}

void previewThreadMain(PreviewState* state) {
    try {
        ...
        while (true) {
            PreviewPacket packet;
            {
                ...
                if (state->stopping && state->generation == seen_generation) break;
            ...
            cv::Mat gray = cv::imdecode(packet.jpeg, cv::IMREAD_GRAYSCALE);
            if (gray.empty()) continue;

            cv::Mat display;
            cv::cvtColor(gray, display, cv::COLOR_GRAY2BGR);
            drawReticleAndTimer(display, packet);
        ...
        std::cerr << "[PREVIEW] OpenCV error: " << e.what() << '\n';
    }
    cv::destroyWindow(PREVIEW_WINDOW);
}

void submitPreview(PreviewState* state,
                   ...
                   uint32_t sequence) {
    PreviewPacket packet;
    const auto* begin = static_cast<const unsigned char*>(jpeg_data);
    packet.jpeg.assign(begin, begin + jpeg_size);
    packet.elapsed_sec = elapsed_sec;
    packet.duration_sec = duration_sec;
    packet.sequence = sequence;

    {
        std::lock_guard<std::mutex> lock(state->mutex);
        state->latest = std::move(packet); // latest-frame-only: preview can never backlog capture.
        ++state->generation;
    }
    state->cv.notify_one();
}

void stopPreview(PreviewState* state, std::thread* thread) {
    {
        std::lock_guard<std::mutex> lock(state->mutex);
        state->stopping = true;
    }
...
void setCameraControl(int fd, uint32_t id, int32_t value, const char* name) {
    v4l2_control ctrl{};
    ctrl.id = id;
    ctrl.value = value;
...
void configureCamera(int fd) {
    ...
    if (xioctl(fd, VIDIOC_S_PARM, &parm) == -1) fail("VIDIOC_S_PARM");

    const auto& tpf = parm.parm.capture.timeperframe;
    const double actual_fps = tpf.numerator
        ? static_cast<double>(tpf.denominator) / static_cast<double>(tpf.numerator)
        : 0.0;
    std::cout << "[CAM] 640x480 MJPEG requested=120 actual="
              << std::fixed << std::setprecision(3) << actual_fps << " FPS\n";
    ...
    setCameraControl(fd, V4L2_CID_GAIN, CAMERA_GAIN, "gain");
...
std::vector<CameraBuffer> initCameraBuffers(int fd) {
    ...
    if (req.count < 4) throw std::runtime_error("Not enough V4L2 buffers");
    ...
    for (uint32_t i = 0; i < req.count; ++i) {
        ...
        if (xioctl(fd, VIDIOC_QBUF, &buf) == -1) fail("VIDIOC_QBUF");
    }
    return buffers;
}

void discardCameraWarmup(int fd, int count) {
    std::cout << "[CAM] discarding " << count << " warmup frames...\n";
    int n = 0;
    while (n < count) {
        pollfd pfd{};
        pfd.fd = fd;
        ...
        if (rc == 0) throw std::runtime_error("Camera warmup timeout");
        ...
        ++n;
        if (xioctl(fd, VIDIOC_QBUF, &buf) == -1) fail("VIDIOC_QBUF warmup");
    }
}
...
void requestHighresImuRate(int fd, uint8_t target_system,
                           uint8_t target_component, int rate_hz) {
    ...
    mavlink_msg_command_long_pack(
        ...
        MAVLINK_MSG_ID_HIGHRES_IMU, interval_us,
        0, 0, 0, 0, 0);
    sendMavlinkMessage(fd, msg);
}

void sendTimeSyncRequest(int fd, int64_t t0_ns,
                         uint8_t target_system, uint8_t target_component) {
    mavlink_message_t msg{};
    mavlink_msg_timesync_pack(
        COMPANION_SYSID, COMPANION_COMPID, &msg,
        0, t0_ns, target_system, target_component);
    sendMavlinkMessage(fd, msg);
}
...
ClockMapping estimateClockMapping(const std::vector<TimeSyncSample>& samples) {
    ...
    for (const auto* s : good) {
        mean_x += static_cast<long double>(s->fc_ns - fc0);
        mean_y += static_cast<long double>(s->rpi_mid_ns - rpi0);
    }
    ...
    for (const auto* s : good) {
        const long double x = static_cast<long double>(s->fc_ns - fc0);
        const long double y = static_cast<long double>(s->rpi_mid_ns - rpi0);
    ...
    m.drift_ppm = static_cast<double>((m.a - 1.0L) * 1.0e6L);
    m.samples = good.size();
    return m;
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const int64_t heartbeat_deadline = monotonicNs() + 5000000000LL;
        ...
        while (monotonicNs() < heartbeat_deadline) {
            ...
            for (ssize_t i = 0; i < n; ++i) {
                ...
                if (mav_message.msgid == MAVLINK_MSG_ID_HEARTBEAT) {
            ...
            if (have_heartbeat) break;
        }
        if (!have_heartbeat) throw std::runtime_error("MAVLink HEARTBEAT not received");

        std::cout << "[MAV] connected system=" << static_cast<int>(target_system)
                  << " component=" << static_cast<int>(target_component) << '\n';
        requestHighresImuRate(serial_fd, target_system, target_component, IMU_RATE_HZ);
        imu_rate_requested = true;
        std::cout << "[MAV] HIGHRES_IMU requested @ " << IMU_RATE_HZ << " Hz\n";

        camera_fd = open(CAMERA_DEVICE, O_RDWR | O_NONBLOCK);
        if (camera_fd == -1) fail("open camera");
        configureCamera(camera_fd);
        camera_buffers = initCameraBuffers(camera_fd);

        v4l2_buf_type camera_type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        if (xioctl(camera_fd, VIDIOC_STREAMON, &camera_type) == -1)
            fail("VIDIOC_STREAMON");
        camera_streaming = true;
        discardCameraWarmup(camera_fd, CAMERA_WARMUP_FRAMES);
        ...
        std::ofstream mjpeg(OUTPUT_MJPEG, std::ios::binary | std::ios::trunc);
        if (!mjpeg) throw std::runtime_error("Cannot create MJPEG output");

        camera_samples.reserve(static_cast<size_t>(duration_sec * CAMERA_FPS + 100));
        imu_samples.reserve(static_cast<size_t>(duration_sec * IMU_RATE_HZ + 100));
        timesync_samples.reserve(static_cast<size_t>(duration_sec * TIMESYNC_RATE_HZ + 100));
        ...
        preview_thread = std::thread(previewThreadMain, &preview_state);
        ...
        std::cout
            ...
            << "preview:        live <=30 FPS, timer + center reticle\n"
        ...
        while (monotonicNs() < end_ns && !preview_state.stop_requested.load()) {
            const int64_t now_ns = monotonicNs();
            if (now_ns >= next_timesync_ns && pending_timesync_t0 == 0) {
                const int64_t t0 = monotonicNs();
                sendTimeSyncRequest(serial_fd, t0, target_system, target_component);
                pending_timesync_t0 = t0;
                next_timesync_ns = t0 + TIMESYNC_PERIOD_NS;
            }
            ...
            if (pfds[0].revents & POLLIN) {
                while (true) {
                    ...
                    const int64_t v4l2_ns = timevalToNs(buf.timestamp);
                    const int64_t corrected_ns =
                        jtzero::timesync::correctCameraTimestampNs(v4l2_ns);

                    if (have_camera_sequence) {
                        const uint32_t expected = previous_camera_sequence + 1;
                        if (buf.sequence != expected)
                            camera_source_drops += static_cast<uint32_t>(buf.sequence - expected);
                    }
                    previous_camera_sequence = buf.sequence;
                    have_camera_sequence = true;

                    if (buf.index >= camera_buffers.size())
                        throw std::runtime_error("Invalid V4L2 buffer index");
                    if (buf.bytesused == 0 || buf.bytesused > camera_buffers[buf.index].length)
                        throw std::runtime_error("Invalid MJPEG buffer size");

                    const std::streampos file_pos = mjpeg.tellp();
                    if (file_pos < 0) throw std::runtime_error("MJPEG tellp failed");
                    const uint64_t mjpeg_offset = static_cast<uint64_t>(file_pos);

                    const void* frame_ptr = camera_buffers[buf.index].start;
                    mjpeg.write(static_cast<const char*>(frame_ptr),
                                static_cast<std::streamsize>(buf.bytesused));
                    if (!mjpeg) throw std::runtime_error("MJPEG write failed");

                    camera_samples.push_back({recv_ns, v4l2_ns, corrected_ns,
                                              buf.sequence, buf.flags,
                                              buf.bytesused, mjpeg_offset});

                    if ((buf.sequence % PREVIEW_DIVIDER) == 0) {
                        const double elapsed_sec =
                            static_cast<double>(recv_ns - start_ns) * 1.0e-9;
                        submitPreview(&preview_state, frame_ptr, buf.bytesused,
                                      elapsed_sec, duration_sec, buf.sequence);
                    }
                ...
                while (true) {
                    ...
                    for (ssize_t i = 0; i < n; ++i) {
                        ...
                        if (mav_message.msgid == MAVLINK_MSG_ID_HIGHRES_IMU) {
                            ...
                            mavlink_msg_highres_imu_decode(&mav_message, &imu);
                            ImuSample s{};
                            s.recv_ns = recv_ns;
                            s.fc_ns = static_cast<int64_t>(imu.time_usec) * 1000LL;
                            s.xacc = imu.xacc;
                            s.yacc = imu.yacc;
                            s.zacc = imu.zacc;
                            s.xgyro = imu.xgyro;
                            ...
                            s.fields_updated = imu.fields_updated;
                        ...
                        } else if (mav_message.msgid == MAVLINK_MSG_ID_TIMESYNC) {
                            mavlink_timesync_t ts{};
                            mavlink_msg_timesync_decode(&mav_message, &ts);
                            if (ts.tc1 != 0 && pending_timesync_t0 != 0 &&
                                ts.ts1 == pending_timesync_t0) {
                                TimeSyncSample s{};
                                s.t0_rpi_ns = pending_timesync_t0;
                                s.t1_rpi_ns = recv_ns;
                                s.fc_ns = ts.tc1;
                                s.rtt_ns = s.t1_rpi_ns - s.t0_rpi_ns;
                                s.rpi_mid_ns = s.t0_rpi_ns + s.rtt_ns / 2;
                                s.good = s.rtt_ns > 0 &&
                                         nsToMs(s.rtt_ns) <= MAX_TIMESYNC_RTT_MS;
            ...
            if (pending_timesync_t0 != 0 &&
                monotonicNs() - pending_timesync_t0 > 20000000LL)
                pending_timesync_t0 = 0;
        }

        stopPreview(&preview_state, &preview_thread);
        preview_started = false;

        mjpeg.flush();
        mjpeg.close();

        const ClockMapping mapping = estimateClockMapping(timesync_samples);
        if (!mapping.valid) throw std::runtime_error("Not enough valid TIMESYNC samples");
        ...
        for (size_t i = 0; i < camera_samples.size(); ++i) {
            camera_latency_ms.push_back(nsToMs(camera_samples[i].recv_ns -
                                               camera_samples[i].v4l2_ns));
            if (i > 0)
                camera_dt_ms.push_back(nsToMs(camera_samples[i].v4l2_ns -
                                              camera_samples[i - 1].v4l2_ns));
        }
        for (const auto& s : imu_samples) {
            const int64_t mapped_ns = mapping.map(s.fc_ns);
            imu_transport_ms.push_back(nsToMs(s.recv_ns - mapped_ns));
        }
        size_t good_timesync = 0;
        for (const auto& s : timesync_samples) {
            if (s.good) {
                ++good_timesync;
                timesync_rtt_ms.push_back(nsToMs(s.rtt_ns));
            }
        }

        std::ofstream camera_index(OUTPUT_CAMERA_INDEX);
        if (!camera_index) throw std::runtime_error("Cannot create camera index CSV");
        camera_index
        ...
        for (const auto& s : camera_samples) {
            camera_index << s.sequence << ',' << s.v4l2_ns << ','
                         << s.corrected_ns << ',' << s.recv_ns << ','
                         << nsToMs(s.recv_ns - s.v4l2_ns) << ','
                         << s.mjpeg_offset << ',' << s.bytes_used << ','
                         << cameraTimestampFlags(s.flags) << '\n';
        }
        camera_index.close();

        std::ofstream csv(OUTPUT_CSV);
        if (!csv) throw std::runtime_error("Cannot create output CSV");
        csv
            << "event,recv_rpi_ns,source_timestamp_ns,mapped_rpi_ns,transport_latency_ms,"
            << "camera_sequence,camera_flags,camera_bytes,"
            << "xacc_m_s2,yacc_m_s2,zacc_m_s2,xgyro_rad_s,ygyro_rad_s,zgyro_rad_s,"
            << "temperature_c,imu_id,timesync_t0_rpi_ns,timesync_t1_rpi_ns,"
            << "timesync_fc_ns,timesync_mid_rpi_ns,timesync_rtt_ms,timesync_good,"
            << "map_a,map_drift_ppm,map_fc_ref_ns,map_rpi_ref_ns\n"
            << std::fixed << std::setprecision(9);

        for (const auto& s : camera_samples) {
            csv << "CAMERA," << s.recv_ns << ',' << s.v4l2_ns << ','
                << s.corrected_ns << ',' << nsToMs(s.recv_ns - s.v4l2_ns) << ','
                << s.sequence << ',' << cameraTimestampFlags(s.flags) << ','
                << s.bytes_used << ",,,,,,,,,,,,,,"
                << static_cast<double>(mapping.a) << ',' << mapping.drift_ppm << ','
                << mapping.fc_ref_ns << ','
                << static_cast<int64_t>(std::llround(mapping.rpi_ref_ns)) << '\n';
        }
        for (const auto& s : imu_samples) {
            const int64_t mapped_ns = mapping.map(s.fc_ns);
            csv << "IMU," << s.recv_ns << ',' << s.fc_ns << ',' << mapped_ns << ','
                << nsToMs(s.recv_ns - mapped_ns) << ",,,,"
                << s.xacc << ',' << s.yacc << ',' << s.zacc << ','
                << s.xgyro << ',' << s.ygyro << ',' << s.zgyro << ','
                << s.temperature << ',' << static_cast<int>(s.imu_id)
                << ",,,,,," << static_cast<double>(mapping.a) << ','
                << mapping.drift_ppm << ',' << mapping.fc_ref_ns << ','
                << static_cast<int64_t>(std::llround(mapping.rpi_ref_ns)) << '\n';
        }
        for (const auto& s : timesync_samples) {
            csv << "TIMESYNC," << s.t1_rpi_ns << ",,,,,,,,,,,,,,,"
                << s.t0_rpi_ns << ',' << s.t1_rpi_ns << ',' << s.fc_ns << ','
                << s.rpi_mid_ns << ',' << nsToMs(s.rtt_ns) << ','
                << (s.good ? 1 : 0) << ',' << static_cast<double>(mapping.a) << ','
                << mapping.drift_ppm << ',' << mapping.fc_ref_ns << ','
                << static_cast<int64_t>(std::llround(mapping.rpi_ref_ns)) << '\n';
        }
        csv.close();
        ...
        std::cout
            ...
            << "============================================================\n"
            << std::setprecision(12)
            << "A (RPi/FC):           " << static_cast<double>(mapping.a) << '\n'
            << std::setprecision(3)
            << "drift:                " << mapping.drift_ppm << " ppm\n"
            << "TIMESYNC good:         " << good_timesync << "/"
            << timesync_samples.size() << '\n';
        if (!timesync_rtt_ms.empty()) {
            std::cout << "TIMESYNC RTT median:   " << percentile(timesync_rtt_ms, 0.50) << " ms\n"
                      << "TIMESYNC RTT p95:      " << percentile(timesync_rtt_ms, 0.95) << " ms\n";
        }
        ...
        std::cout
            ...
            << "CAMERA\n"
            << "============================================================\n"
            << "frames:                " << camera_samples.size() << '\n'
            << "source drops:          " << camera_source_drops << '\n';
        if (!camera_latency_ms.empty()) {
            std::cout << "delivery median:       " << percentile(camera_latency_ms, 0.50) << " ms\n"
                      << "delivery p95:          " << percentile(camera_latency_ms, 0.95) << " ms\n"
                      << "delivery p99:          " << percentile(camera_latency_ms, 0.99) << " ms\n"
                      << "delivery max:          "
                      << *std::max_element(camera_latency_ms.begin(), camera_latency_ms.end())
                      << " ms\n";
        }
        if (!camera_dt_ms.empty()) {
            std::cout << "timestamp dt median:   " << percentile(camera_dt_ms, 0.50) << " ms\n"
                      << "timestamp dt p95:      " << percentile(camera_dt_ms, 0.95) << " ms\n";
        }
        ...
        if (!imu_transport_ms.empty()) {
            std::cout << "transport median:      " << percentile(imu_transport_ms, 0.50) << " ms\n"
                      << "transport p95:         " << percentile(imu_transport_ms, 0.95) << " ms\n"
                      << "transport p99:         " << percentile(imu_transport_ms, 0.99) << " ms\n"
                      << "transport min:         "
                      << *std::min_element(imu_transport_ms.begin(), imu_transport_ms.end()) << " ms\n"
                      << "transport max:         "
                      << *std::max_element(imu_transport_ms.begin(), imu_transport_ms.end()) << " ms\n";
        }
        ...
        if (preview_state.stop_requested.load())
            std::cout << "[PREVIEW] Test stopped by Q/ESC.\n";

        if (imu_rate_requested) {
            requestHighresImuRate(serial_fd, target_system, target_component, 0);
        ...
        if (serial_fd != -1 && imu_rate_requested && target_system != 0) {
            try {
                requestHighresImuRate(serial_fd, target_system, target_component, 0);
            } catch (...) {}
        }
        if (camera_fd != -1 && camera_streaming) {
            v4l2_buf_type type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
            xioctl(camera_fd, VIDIOC_STREAMOFF, &type);
        }
        for (auto& b : camera_buffers)

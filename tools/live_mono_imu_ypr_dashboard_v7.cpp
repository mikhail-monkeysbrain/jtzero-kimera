// JT-ZERO coordinated YAW/PITCH/ROLL dashboard v7.
// Event-driven manual test: no timers. All three axes must be inside target zones.
// Russian GUI with live camera, simultaneous Y/P/R bars and automatic transitions.
...
constexpr const char* kCsv = "/home/vio/jtzero_live_ypr_dashboard_v7.csv";
constexpr const char* kWindow = "JT-ZERO: YAW PITCH ROLL v7";
...
struct State {
  ...
  double ax=0,ay=0,az=0,awx=0,awy=0,awz=0,awxy=0,pred_vxy=0;
  int step=0;
};
 ...
 public:
  ...
  void installCallback(){
    registerBackendOutputCallback([this](const std::shared_ptr<VIO::BackendOutput>& out){
      ...
      const gtsam::Vector3 ab(s.ax-s.bax,s.ay-s.bay,s.az-s.baz);
      const gtsam::Vector3 aw=R*ab+gtsam::Vector3(0,0,-9.81);
...
void renderHud(const cv::Mat& gray,const Pipeline& pipe,const Ref& ref,int step,int stable,bool ready,bool done){
  ...
  cv::Mat canvas(900,1440,CV_8UC3,bg),video;
  ...
  uiText(canvas,"JT-ZERO YAW PITCH ROLL TEST v7",20,48,.78,white,2);
  ...
  else if(done){uiText(canvas,"ТЕСТ ЗАВЕРШЁН",250,305,1.10,green,3);}
  ...
  cv::Mat side=canvas(cv::Rect(860,70,560,810));cv::rectangle(side,{0,0},{559,809},cv::Scalar(45,45,45),1);
  ...
  if(have){char b[200];std::snprintf(b,sizeof(b),"VIO Y/P/R  %+.1f  %+.1f  %+.1f",a.vio_yaw,a.vio_pitch,a.vio_roll);uiText(side,b,18,650,.40,white,1);std::snprintf(b,sizeof(b),"WORLD XY %.3f м/с²",s.awxy);uiText(side,b,18,682,.42,s.awxy>.5?red:white,1);std::snprintf(b,sizeof(b),"Vxy %.3f   PIM PredVxy %.3f м/с",std::hypot(s.vx,s.vy),s.pred_vxy);uiText(side,b,18,714,.40,white,1);}
  uiText(side,"Все три полосы должны быть зелёными",18,760,.40,yellow,1);
  uiText(canvas,"SPACE — ноль   ESC / Q — прервать",25,860,.42,muted,1);uiText(canvas,std::string("CSV: ")+kCsv,720,860,.40,muted,1);
  cv::imshow(kWindow,canvas);
}

void saveCsv(const std::vector<State>& ss,const Ref& ref){
  std::ofstream f(kCsv,std::ios::trunc);
  f<<"step,target_yaw,target_pitch,target_roll,tol_yaw,tol_pitch,tol_roll,wall_ns,keyframe,timestamp_ns,fc_droll,fc_dpitch,fc_dyaw,vio_droll,vio_dpitch,vio_dyaw,err_yaw,err_pitch,err_roll,awx,awy,awz,awxy,vxy,pred_vxy,bax,bay,baz,px,py,pz\n";
  ...
  for(const auto&s:ss){Att a=relativeAtt(s,ref);int idx=std::max(0,std::min((int)steps().size()-1,s.step));const auto&t=steps()[idx];
    f<<s.step<<','<<t.yaw<<','<<t.pitch<<','<<t.roll<<','<<t.yt<<','<<t.pt<<','<<t.rt<<','<<s.wall_ns<<','<<s.keyframe<<','<<s.timestamp_ns<<','
     ...
     <<(a.yaw-t.yaw)<<','<<(a.pitch-t.pitch)<<','<<(a.roll-t.roll)<<','<<s.awx<<','<<s.awy<<','<<s.awz<<','<<s.awxy<<','<<std::hypot(s.vx,s.vy)<<','<<s.pred_vxy<<','<<s.bax<<','<<s.bay<<','<<s.baz<<','<<s.px<<','<<s.py<<','<<s.pz<<'\n';}
}
...
int main(int argc,char**argv){
  ...
  try{
    ...
    std::cout<<"\nJT-ZERO YPR DASHBOARD v7\nNo timers. SPACE fixes zero. Every step requires YAW/PITCH/ROLL simultaneously in green zones.\n";
    while(true){

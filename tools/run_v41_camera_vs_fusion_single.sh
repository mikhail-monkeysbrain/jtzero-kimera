...
echo "3) SPACE или ENTER в активном GUI на A -> плавно двигать к B."
echo "4) Попасть к B примерно за 7.5 с."
echo "5) На физическом упоре B нажать SPACE или ENTER и остановить стенд."
...
if [[ ! -x "${BIN}" || "${ROOT}/tools/live_mono_imu_500mm_v41_camera_vs_fusion.cpp" -nt "${BIN}" || "${ROOT}/tools/v25_parts/live_mono_imu_500mm_repeat_hud_v25_part01.inc" -nt "${BIN}" || "${ROOT}/tools/v18_parts/live_mono_imu_500mm_repeat_hud_v18_part07b.inc" -nt "${BIN}" || "${ROOT}/tools/v25_parts/live_mono_imu_500mm_repeat_hud_v25_part08a.inc" -nt "${BIN}" || "${ROOT}/tools/v25_parts/live_mono_imu_500mm_repeat_hud_v25_part08b.inc" -nt "${BIN}" ]]; then

...
constexpr double ROLL_PITCH_TOL_DEG=5.0,YAW_TOL_DEG=7.0,ZERO_ACQUIRE_SEC=2.0,ZERO_STABILITY_DEG=1.5;
...
struct PreviewState{
    ...
    std::atomic<bool>stop_requested{false},plan_ready{false},attitude_valid{false},zero_locked{false},test_complete{false};
    std::atomic<double>roll_deg{0},pitch_deg{0},yaw_deg{0},roll_zero_deg{0},pitch_zero_deg{0},yaw_zero_deg{0},zero_progress_sec{0};
...
void previewThreadMain(PreviewState*st){
    try{
        ...
        bool zero_started=false;double zero_start_wall=0,last_sample_wall=0,sum_r=0,sum_p=0,sum_sin_y=0,sum_cos_y=0;size_t zero_samples=0;double anchor_r=0,anchor_p=0,anchor_y=0;
        while(true){
            ...
            if(!st->plan_ready.load()){
                drawSetup(c,edit);cv::imshow(PREVIEW_WINDOW,c);int k=cv::waitKeyEx(1);if(k==27){st->stop_requested.store(true);break;}if(k==13||k==10){{std::lock_guard<std::mutex>lk(st->mutex);st->plan_text=edit;st->steps=parsePlan(edit);}st->plan_ready.store(true);zero_started=false;st->zero_locked.store(false);st->zero_progress_sec.store(0);lastWall=monotonicNs()/1e9;}else if(k==65471||k==0x710000)edit=DEFAULT_PLAN;else if(k==8||k==127){if(!edit.empty())edit.pop_back();}else if(k>=32&&k<=126)edit.push_back((char)k);continue;
            }
            ...
            if(!st->zero_locked.load()){
                double r=st->roll_deg.load(),pi=st->pitch_deg.load(),y=st->yaw_deg.load();
                if(!st->attitude_valid.load()){
                    putOutlined(c,"WAITING FOR ATTITUDE",{25,52},.9,2);st->zero_progress_sec.store(0);
                }else{
                    if(!zero_started){zero_started=true;zero_start_wall=now;last_sample_wall=0;sum_r=sum_p=sum_sin_y=sum_cos_y=0;zero_samples=0;anchor_r=r;anchor_p=pi;anchor_y=y;}
                    bool stable=std::abs(wrap180(r-anchor_r))<=ZERO_STABILITY_DEG&&std::abs(wrap180(pi-anchor_p))<=ZERO_STABILITY_DEG&&std::abs(wrap180(y-anchor_y))<=ZERO_STABILITY_DEG;
                    if(!stable){zero_start_wall=now;last_sample_wall=0;sum_r=sum_p=sum_sin_y=sum_cos_y=0;zero_samples=0;anchor_r=r;anchor_p=pi;anchor_y=y;}
                    if(last_sample_wall==0||now-last_sample_wall>=0.015){sum_r+=r;sum_p+=pi;sum_sin_y+=std::sin(y*PI/180.0);sum_cos_y+=std::cos(y*PI/180.0);++zero_samples;last_sample_wall=now;}
                    double progress=std::clamp(now-zero_start_wall,0.0,ZERO_ACQUIRE_SEC);st->zero_progress_sec.store(progress);
                    std::ostringstream z;z<<"HOLD STILL - ACQUIRING ATTITUDE ZERO   "<<std::fixed<<std::setprecision(1)<<progress<<" / "<<ZERO_ACQUIRE_SEC<<" s";putOutlined(c,z.str(),{25,52},.75,2);
                    std::ostringstream raw;raw<<"RAW  R "<<std::fixed<<std::setprecision(1)<<r<<"   P "<<pi<<"   Y "<<y;putOutlined(c,raw.str(),{820,220},.62,1);
                    if(progress>=ZERO_ACQUIRE_SEC&&zero_samples>0){st->roll_zero_deg.store(sum_r/zero_samples);st->pitch_zero_deg.store(sum_p/zero_samples);st->yaw_zero_deg.store(rad2deg(std::atan2(sum_sin_y,sum_cos_y)));st->zero_locked.store(true);st->step_index.store(0);st->step_collected_sec.store(0);lastWall=now;}
                }
                cv::putText(c,"Keep the stand motionless. Movement resets the 2 s zero timer.",{820,300},cv::FONT_HERSHEY_SIMPLEX,.48,{0,220,255},1,cv::LINE_AA);cv::putText(c,"Q / ESC = stop",{820,620},cv::FONT_HERSHEY_SIMPLEX,.6,{255,255,255},1);cv::imshow(PREVIEW_WINDOW,c);int k=cv::waitKeyEx(1);if(k==27||k=='q'||k=='Q')st->stop_requested.store(true);continue;
            }
            ...
            if(idx>=(int)steps.size()){st->test_complete.store(true);putOutlined(c,"CALIBRATION SEQUENCE COMPLETE",{25,52},.9,2);}
                ...
                Step s=steps[idx];double r=wrap180(st->roll_deg.load()-st->roll_zero_deg.load()),pi=wrap180(st->pitch_deg.load()-st->pitch_zero_deg.load()),y=wrap180(st->yaw_deg.load()-st->yaw_zero_deg.load());bool anglesGood=st->attitude_valid.load()&&attitudeInZone(r,pi,y,s);double collected=st->step_collected_sec.load();if(anglesGood)collected+=dt;st->step_collected_sec.store(collected);
                ...
                cv::putText(c,"RELATIVE ATTITUDE / TARGET",{820,130},cv::FONT_HERSHEY_SIMPLEX,.72,{255,255,255},2);drawGauge(c,820,220,410,"ROLL d",r,s.roll,60,ROLL_PITCH_TOL_DEG);drawGauge(c,820,320,410,"PITCH d",pi,s.pitch,60,ROLL_PITCH_TOL_DEG);drawGauge(c,820,420,410,"YAW d",y,s.yaw,90,YAW_TOL_DEG);
                std::ostringstream zeros;zeros<<"ZERO: R "<<std::fixed<<std::setprecision(1)<<st->roll_zero_deg.load()<<"  P "<<st->pitch_zero_deg.load()<<"  Y "<<st->yaw_zero_deg.load();cv::putText(c,zeros.str(),{820,500},cv::FONT_HERSHEY_SIMPLEX,.46,{180,180,180},1,cv::LINE_AA);
                cv::putText(c,anglesGood?"COLLECTING DATA":"MOVE INTO GREEN ZONES - TIMER PAUSED",{820,555},cv::FONT_HERSHEY_SIMPLEX,.5,anglesGood?cv::Scalar(0,255,0):cv::Scalar(0,180,255),2,cv::LINE_AA);
            }
